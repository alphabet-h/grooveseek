...
**plugin を差し替えたら `--force` で index し直すこと。** grammar を作り直すと同じファイルでも chunk の切れ目が動きうるが、索引は内容が変わっていないファイルを飛ばすので、普通に index し直しても**そのファイルは古い grammar が切った chunk のまま**残り、新しい grammar はその後編集したファイルにだけ効く。groove は「どの build がこの索引の chunk を切ったか」を記録していて、一致しなくなったら警告する。実際に切り直すのは `groove index --force`。`[parsers.code].max_chunk_chars` を変えた時も同じ理由で同じ。
