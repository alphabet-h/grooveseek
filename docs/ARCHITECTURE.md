...
| `src/parser/` | Parser trait + Registry. `mod.rs` (Frontmatter / Chunk / ParsedDocument, plus the `parse_bytes(bytes, path_hint, exclude_headings) -> Result<ParsedDocument>` entry point that every call site — indexer and server — now goes through; default impl validates UTF-8 then delegates to `parse`, so `md`/`txt` need no override, while future binary-format parsers override it directly. `is_binary()` (default `false`) flags binary parsers for `get_document`'s size-cap classification and quality-filter exemption), `markdown.rs`, `txt.rs`, `registry.rs` (extension lookup). |
