...
**FTS 半身のコスト。** `ORDER BY bm25(...)` は `LIMIT` を適用する**前に**マッチした全行をスコアリングするため、全文検索側のコストは「要求した件数」ではなく「式がマッチする行数」に比例する。実測 (release ビルド、1 つの phrase が全行に当たる合成 corpus): 5,000 行で 3.3 ms、20,000 行で 15.5 ms、40,000 行で 29.4 ms = 照合行数に線形。32 phrase の `OR` は同じ母集団に対して単一 phrase の **約 2 倍** (上記サイズでそれぞれ 2.3 / 1.8 / 2.0 倍) で、この倍率は corpus が増えても増えない — phrase は 1 回のスキャンを広げるだけで、phrase ごとにスキャンを始めるわけではないため。

つまみを触る前に知っておくべき帰結が 2 つ。第 1 に、**上限値を下げてもこのコストは減らない** — 同じクエリが `LIMIT 1` でも `LIMIT 100` でも同じ時間で、効くのは数千行を実際に**返す**ほど大きな上限のとき (行の実体化) だけ。第 2 に、index の全行にマッチさせること自体は昔から一般的な部分文字列 1 つで可能だった (`"について"` 単独で全行)。token 単位のコンパイルは上限を上げたのではなく、普通のクエリがそこに届く確率を上げただけ。回帰ガードは絶対時間ではなく**倍率**を固定している (`bu03_or_expansion_stays_within_a_small_multiple_of_a_single_phrase`)。
