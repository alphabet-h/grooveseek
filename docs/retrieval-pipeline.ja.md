...
つまみを触る前に知っておくべき帰結が 3 つ。第 1 に、**上限値を下げてもこのコストは減らない** — 40,000 行のとき `LIMIT 1` で 339 ms、`LIMIT 100` で 329 ms。効くのは数千行を実際に**返す**ほど大きな上限のとき (10,000 で +42 ms) だけで、それは実体化のコストであって照合ではない。第 2 に、index の全行にマッチさせること自体は昔から一般的な部分文字列 1 つで可能だった (`"について"` 単独で全行)。token 単位のコンパイルは「1 クエリが触れる行数」の上限を上げたのではないが、**コストの上限は約 10 倍に上げた**。第 3 に、最悪コストを実際に縛れるつまみは phrase 上限 (32) で、コストが phrase 数にほぼ線形だから効く。ただし feature-48 の検索品質評価がこの値で測られているため据え置いてある — 下げると長いクエリの recall と引き換えになり、その代償はまだ測っていない。
