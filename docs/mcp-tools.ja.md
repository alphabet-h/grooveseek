...
| `get_document` | 相対パスから文書の全文 + メタデータを取得。パスはその文書の**ただ 1 つの綴り** — KB 相対・`/` 区切りで、`search` が返したとおり — でなければならない。同じファイルに届く別の書き方 (`./a.md`、`a//b.md`、`a/../a.md`、バックスラッシュ、絶対パス、Windows / macOS での大文字小文字違い、8.3 短縮名、symlink のディレクトリ経由) は "not found" になる。要求文字列で絞り込む前段ゲートウェイを迂回させないため | `path` (例: `"deep-dive/mcp/overview.md"`) |
