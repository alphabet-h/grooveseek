...
| `get_document` | 相対パスから文書の全文 + メタデータを取得。パスはその文書の**ただ 1 つの綴り** — KB 相対・`/` 区切りで、`search` が返したとおり — でなければならない。同じファイルに届く別の書き方 (`./a.md`、`a//b.md`、`a/../a.md`、絶対パス、Windows / macOS での大文字小文字違い、Windows でのバックスラッシュと 8.3 短縮名、symlink のディレクトリ経由) は "not found" になる。要求文字列で絞り込む前段ゲートウェイを迂回させないため。KB の中を指し得ないパス (絶対パス・空文字列・`..` という区間を含むもの、Windows ではドライブ指定・UNC 共有・バックスラッシュ・どこかに含まれる `:` (`note.md:secret` のような代替データストリーム)・`NUL` や `COM1` などの予約デバイス名) はディスクを見る前に拒否する: そこに何かがあっても無くても答えは同じで、Windows で共有に接続することもない。KB の中を指す別の綴り (`./a.md`、`a//b.md`) は先にディスクを見てから (見るのは KB の中だけ) 同じく "not found" になる | `path` (例: `"deep-dive/mcp/overview.md"`) |
