...
| `kb-mcp/src/server.rs` | rmcp `ServerHandler` 実装。6 つの MCP ツールをディスパッチ。`search` は `db.search_hybrid` 経由で結果を `SearchResponse` ラッパ (`low_confidence` / `match_spans` / `filter_applied`) に包んで返す (v0.3.0 で BREAKING、CHANGELOG 参照)。状態は `KbCore` が持ち、各ツールは本体を `spawn_blocking` でそこへ流す薄い `async` ラッパ。長い処理は async worker ではなく blocking スレッドを占有する |
...
- **`chunks.context_text TEXT`** (`kb-mcp/src/db/schema.rs`): nullable 列。feature-46 以前の DB には idempotent な `ensure_context_text_column` の `ALTER TABLE` で追加される。現在の `ContextMode` が `Static` のときのみ `Chunk.context` から埋まり、`Off` モードでは `NULL` のまま。
