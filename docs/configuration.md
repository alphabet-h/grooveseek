...
| `max_retries` | Integer 0-10 (openai-compatible only) | `3` | How many times a batch is sent again after HTTP 429, a 5xx, a timeout or a failed connection. Waits 1 s, 2 s, 4 s ... (capped at 60 s) or exactly what `Retry-After` asks; a `Retry-After` over 60 s fails at once. `0` sends once. |
