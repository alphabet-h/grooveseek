...
約束になる。それは本プロジェクトに守れる約束ではない。観測できる面は、public な
Rust API 全体、すべてのコマンドラインフラグ、**6 個の MCP tool**、`groove.toml` の
全セクション、そして SQLite スキーマに及ぶ。全部を凍らせれば、誰も依存していない
部分を永久に改善できないか、
...
- `validate` の JSON が持つ `kind` の語彙は追加のみで増える: `kind` で分岐する
  消費側は未知の値を素通りさせなければならない。`frontmatter_unparsed` (1.8.0) と
  `undeclared_field` (1.9.0) はその形で追加された
...
| `results[].start_line` | integer | chunk がソースファイル由来のときのみ。他は**不在**。1 始まりで、**由来した定義ではなく返した `content` 自身**の範囲を指す。したがって parent retriever が拡張した hit では merge した全 chunk を覆う範囲になり、それらが揃って行範囲を持たない場合はキーごと**不在**になる |
| `results[].end_line` | integer | 同じ条件で**不在**。1 始まりで終端を含む |
| `results[].symbol_kind` | string | chunk が定義のときのみ。他は**不在**。拡張で 2 つ以上の chunk が merge された hit でも**不在**になる。grammar 自身の語 (`function` / `class` / `method` / `constant` …) で、対応言語が増えると値も増える |
...
- **設定ファイルに前方互換性は無い。** 未知のキーは拒否されるので、1.0.x の
  ...
  **違うモデルでナレッジベースが索引される**。起動しない方が安全な失敗の向きである。
  ...
  共有するものではない
- **同じ設定を 2 度名指しする組み合わせは、拒否されるようになることがある。**
  キーの名前・型・既定値は凍結されたままだが、同じものを決める 2 つのキーの片方が
  黙って勝つのは、ファイルの機能ではなく読み方の bug である。該当する例:
  FastEmbed provider では、トップレベルの `model` と `[embedding].model` がどちらも
  model を選び、`[embedding].model` が勝っていた。両方を書いたファイルは v1.13.0
  以前では読み込めたが、それより後では拒否される。どちらか一方のキーを消すこと。
  [`[embedding]` セクション](configuration.ja.md#embedding-セクション) を参照

### 既定の embedding モデル

既定のモデルを変えることは **major** である。モデル識別子は索引に記録され、
起動時に完全一致が要求されるので、既定を変えると**既存の全インストールが
再索引するまで起動しなくなる**。既定の provider (FastEmbed) を変えることも、
同じ理由で **major** である — 索引に記録された identity が一致しなくなる。
...
| 環境変数 | `GROOVE_CONFIG_HOME` / `GROOVE_TRAY_LOG` / `GROOVE_GRAMMAR_DIR` (v1.3.0+、絶対パス必須) / `GROOVE_EMBEDDING_API_KEY` (`FASTEMBED_CACHE_DIR` は fastembed の名前なので尊重はするが凍結対象にはしない。`GROOVE_BIN` は同梱サンプル hook の変数で、バイナリは読まない) |
