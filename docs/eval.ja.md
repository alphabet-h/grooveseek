...
> 生成過程に対するシミュレーションでは、fold の選択が少しでも割れる限り報告される
> SE は真値の **0.53〜0.60 倍**で、条件 3 は 2 sigma よりも 1.1 sigma 程度の検定
> として働く。比が 1.03 まで戻るのは**全 fold が同じ条件を選んだ場合だけ**であり、
> 条件 4 はこれを救わない — fold の 84% が一致する実行は条件 4 を通過しつつ、SE を
> なお約 1.9 倍過小評価する。係数は**意図的に上げていない** — 上げるとツールの推奨
> 内容そのものが変わるため。条件 1 / 2 / 4 / 5 は影響を受けない。シミュレーションは
> `tune.rs` の `au16_paired_se_versus_the_true_standard_error`。
