...
- 照合は**索引されたテキストフィールド 1 つの中**で行う。chunk 本文と見出しは
  別々に走査し、chunk どうしも別に扱う。見出しを対象にするのは、Markdown parser が
  見出し行を本文から取り除き、かつ全文検索が見出しを**本文より重く**索引する
  ためである。golden query を `##` 見出しに並べるのはテストを記録する最も自然な
  書き方であり、本文だけを見る走査ではそれが丸ごと見えない。何も連結しないので
  継ぎ目をまたぐ引用は見つからないが、これは意図的な選択である — 連結すると
  継ぎ目の両側にある無関係な文が 1 つの引用に見えてしまう。
