...
> **条件 3 は「2 sigma」という語感より緩い。** `SD({d_j}) / sqrt(N)` は fold ごとの
> 差分が独立であることを仮定するが、N 個の leave-one-out 選択は互いに N−2 個の
> query を共有するため正に相関し、この式は真のばらつきを過小評価する。既知の
> 生成過程に対するシミュレーションでは、報告される SE は真値の **0.53〜0.60 倍**
> で、条件 3 は 2 sigma よりも 1.1 sigma 程度の検定として働く。測った 4 設定のうち
> 比が 1.03 になったのは、全 replication で fold が完全一致した 1 設定だった。相関の
> 源は fold が選択を違えることなので機構とは整合するが、完全に安定な設定が 1 つしか
> 無い以上、これは連関の観測であって「一致した時に限り較正される」ことの証明ではない。
>
> **条件 4 は差を縮めるが埋めない。** `stability > 0.5` を通った replication (=
> 採用され得る唯一の部分集合) だけで取り直すと比は **0.62〜0.73** に上がるものの、
> それらの実行でも SE はなお約 1.4〜1.6 倍過小である。稀な隅でもない: 300 rep 中
> 192〜300 が通過している。(比は「replication 群」に対する量で単一 run の性質では
> ないため、平均 stability と並べて読むのではなく、その部分集合で計算し直す必要が
> ある。)
>
> 係数は**意図的に上げていない** — 上げるとツールの推奨内容そのものが変わるため。
> 条件 1 / 2 / 4 / 5 は影響を受けない。シミュレーションは `tune.rs` の
