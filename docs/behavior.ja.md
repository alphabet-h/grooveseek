...
- **壊れた YAML frontmatter は skip ではなく索引される** (v1.7.0+): `---` ブロックが parse できない `.md` は、本文はそのまま、`title` / `date` / `topic` / `depth` は空で索引され、`frontmatter:unparsed` の tag が付き、stderr に `warning: <path>: failed to parse YAML frontmatter: <reason>` と名指しされ、`Done in` summary と MCP `rebuild_index` の stats で `frontmatter unparsed` として数えられる。この文書は filter 無しの検索には出るのに、失った field への filter からは消える — tag があるのはそのためで、`tags_any: ["frontmatter:unparsed"]` で見つかる。`[index].fail_on_frontmatter_error = true` なら、そういう run の後で `groove index` は exit 1 になる (索引はどちらでも書かれる)。閉じていない `---` ブロックはエラーではなく、従来どおり本文として読む
