...
- **(v1.1.0+) A group led by a hyphen at a whitespace boundary is excluded, not searched for.** `rust -async` searches for `rust` and excludes `async`. An unquoted `-word` is tokenized with the same script-boundary and trigram-floor rules as a positive phrase, so `-再ランキング` also excludes `ランキング` — quote it (`-"再ランキング"`) to exclude only the compound; `-ab` excludes nothing, since the excluded phrase is under the trigram floor too. The exclusion joins the compiled positive phrases as `(positives) NOT (negatives)`, judged against the same FTS5 row a positive match sees — `heading`, the contextual prefix, and `content` together, not the body alone — and the vector half drops any candidate whose chunk id is in the set FTS5 returns for the negative expression alone, so both halves agree on what was excluded. `"-foo"` searches for a literal leading hyphen. A query made only of exclusions is refused rather than run. The embedder, the reranker, and `match_spans` all see the query with its exclusions cut out; the parent retriever's later content expansion is not re-checked, so an excluded term can reappear in expanded text. See [ADR-0011](./decisions/0011-exclude-a-term-from-both-halves-of-the-search.md) for why both halves need to agree.

So `再ランキングの評価について` becomes `"再ランキング" OR "ランキング" OR "の評価" OR "について"`, and `"Foundry Local" の設定` becomes `"Foundry Local" OR "の設定"`. Before v0.16.0 the whole query was wrapped in one phrase, which over a trigram tokenizer is a verbatim substring search — a natural-language Japanese query produced zero FTS candidates, so the FTS half of the hybrid was effectively dead and only the vector half ran. Why script boundaries rather than a morphological analyzer, and what the change cost in retrieval quality, is in [ADR-0002](./decisions/0002-compile-queries-into-per-token-fts-phrases.md).

If tokenizing produces no phrase at all — every fragment is under the floor, as in `AI と ML` — the whole trimmed query falls back to that single-phrase form, so this shape of query does not regress. Only a query that is itself under 3 characters after trimming skips FTS entirely and searches vector-only. Quoting the entire query reproduces the old verbatim behavior on demand. This is a query-side change only: the index, the schema, and the tokenizer are untouched, so **no re-index is needed**.
...
**What an exclusion costs (v1.1.0+).** The negative phrases are capped separately, also at 32, so the full-text half's worst case becomes two 32-phrase `OR` expressions joined by `NOT` in one statement rather than one. The vector half adds a single unranked, unlimited `rowid` scan of the same negative expression against `fts_chunks` to build the set of ids to drop — no bm25, no `ORDER BY`. That scan is cheap relative to the ranked query it accompanies: measured at 934.5µs (best of 5) for the id scan against 3.5855ms for the ranked FTS query, both against a negative expression matching every one of 5,000 chunks — the worst case — a ratio of 0.26, pinned by the release-profile test `the_exclusion_id_scan_stays_cheaper_than_the_ranked_fts_query` in `grooveseek/src/db.rs` (details: [ADR-0011](./decisions/0011-exclude-a-term-from-both-halves-of-the-search.md)). A query with no exclusion pays none of this — the scan runs only when there is a negative expression to evaluate.
