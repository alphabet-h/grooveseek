...
**What the FTS half costs.** `ORDER BY bm25(...)` scores *every* matching row before `LIMIT` is applied, so the cost of the full-text half tracks the number of rows the expression matches — not the number you asked for. Measured on this pipeline (release build, synthetic corpora where one phrase matches every row): 3.3 ms at 5,000 rows, 15.5 ms at 20,000, 29.4 ms at 40,000, i.e. linear in the matching population. The 32-phrase `OR` costs about **2×** a single-phrase query over the same population (2.3× / 1.8× / 2.0× at those sizes), and that multiple does not grow with the corpus, because the phrases widen one scan rather than each starting their own.

Two consequences worth knowing before tuning anything. First, **lowering a limit does not reduce this cost**: the same query takes the same time at `LIMIT 1` and at `LIMIT 100`, and only a limit large enough to *return* thousands of rows adds anything (materialising them). Second, matching every row in the index has always been one common substring away — `"について"` alone does it with a single phrase — so per-token compilation did not raise the ceiling, only the chance that an ordinary query reaches it. A regression guard pins the multiple rather than any absolute timing (`bu03_or_expansion_stays_within_a_small_multiple_of_a_single_phrase`).
