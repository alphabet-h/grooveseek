...
| `max_retries` | 0-10 の整数 (openai-compatible 専用) | `3` | HTTP 429・5xx・timeout・接続失敗のあと batch を送り直す回数。待ちは 1 秒・2 秒・4 秒… (60 秒で頭打ち) か `Retry-After` の値ちょうど。60 秒を超える `Retry-After` は待たずに失敗。`0` なら 1 回だけ送る。 |
