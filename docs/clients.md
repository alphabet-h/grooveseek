...
**When you replace a plugin, re-index with `--force`.** A rebuilt grammar can cut the same file into different chunks, but indexing skips files whose content has not changed, so a plain re-index would leave those files with chunks the old grammar made and apply the new one only to files you edit afterwards. groove records what cut the chunks in an index — a hash of each plugin library's bytes, so any rebuild counts, together with a version for groove's own chunking rules — and warns when that is no longer what is loaded; `groove index --force` is what actually re-chunks them. The same warning therefore appears after a groove upgrade that changed how definitions are cut, not only after you replace a plugin. The same is true after changing `[parsers.code].max_chunk_chars`, and for the same reason.
