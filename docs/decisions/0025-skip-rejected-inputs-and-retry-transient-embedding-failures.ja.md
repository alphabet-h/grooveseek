...
4. **拒否のあった run が何を報告するか。**
   ...
   - **拒否があり、かつ endpoint が何も受け付けなかったときだけ exit 非 0。** 採用。
     この形はファイルではなく `model` / `document_model` か `endpoint` を指している。
     endpoint が前のバッチを受け付けた後で断ったファイルは、この拒否ではなく受け付けた
     側に数える: 受け付けたバッチが設定の正しさを示している。
...
- **拒否があり endpoint が何も受け付けなかった run は、最後まで済ませた後で失敗にする**
  (`IndexResult::fails_all_inputs_rejected`)。複数バッチ (64 チャンク超) のファイルを
  endpoint が前のバッチを受け付けた後で断った場合も同じく skip するが、拒否ではなく
  受け付けた側 (`IndexResult::embedded`) に数えるので、そうした run は失敗にならない。
  run を途中で切らない: 削除の sweep と
