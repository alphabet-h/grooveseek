...
- **到達性が groove の知らないものになる**。socket の path・所有者・mode、および
  ...
  **上の背景で挙げた loopback ポートとまったく同じ到達性**である。
  例外は、運用者に設定するものが 1 つも無い場合である: abstract socket —
  名前が `@` で始まる `ListenStream=` — はファイルシステム上の実体を持たないので、
  正しく設定すべき所有者も mode も存在しない。groove は、ホスト上の全アカウントに
  admin 経路を local として開くのではなく、**起動時に拒否する**
