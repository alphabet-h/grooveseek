...
- **plugin の置き場は特権的な設定になる。** 知識ベースの隣で見つかった設定ファイルは
  ...
  `[parsers]` を書かなければ既に Markdown のみだから。)* *(v1.6.0 以降、
  [ADR-0016](0016-keep-the-plugin-directory-outside-the-knowledge-base.ja.md) を参照:
  この帰結が守るのは**誰がその値を書けるか**であり、その値が指す先が知識ベースの内側
  だった場合は別途拒否するようになった。)*
