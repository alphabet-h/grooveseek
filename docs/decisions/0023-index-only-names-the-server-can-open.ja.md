...
- watcher は拡張子の判定の後で、イベントから取り戻した相対パスについて同じ述語に訊く。
  ...
  そこでは拒否される名前は watcher の他の判定 (除外・拡張子・リンク) も飛ばす。
  そういう行をこの版が書いたことはありえないため。
...
- 悪い: 正しい Unicode でない名前 (Unix では不正なバイト列、Windows では対になって
  いないサロゲート) は、復号できなかった部分を U+FFFD に置き換えた綴りで今も索引され、
  `get_document` はその綴りを開けない。述語はその置き換え後の綴りを判定して受け入れる
  ので、上の 1 つ目の「良い」はそういう名前には成り立たない。本 ADR はこれを変えず、
  別件として扱う。
