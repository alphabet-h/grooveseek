...
- **A combination that names one setting twice may start being refused.** Key
  ...
  of the file. The case this was written for: with the FastEmbed provider, the top-level
  `model` and `[embedding].model` both choose the model, and `[embedding].model`
  used to win. A file that sets both loaded in v1.13.0 and earlier and is
  refused after it; remove one of the two keys. See
...
| environment variables | `GROOVE_CONFIG_HOME`, `GROOVE_TRAY_LOG`, `GROOVE_GRAMMAR_DIR` (v1.3.0+; must be absolute), `GROOVE_EMBEDDING_API_KEY`. (`FASTEMBED_CACHE_DIR` is fastembed's name — honoured, but not ours to freeze. `GROOVE_BIN` belongs to the shipped example hook, not to the binary.) |
