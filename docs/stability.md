...
| `results[].symbol_kind` | string | **omitted** unless the chunk is a definition, and on any hit the parent retriever expanded — the grammar's own word (`function`, `class`, `method`, `constant`, …), and the set grows as languages are added |
