...
It also reports **files that stopped being read**. This project was named `kb-mcp` through v0.25.0, and the rename shipped no aliases and no automatic migration ([ADR-0007](decisions/0007-rename-the-project-to-grooveseek.md)) — a `groove` binary does not open anything `kb-mcp` left behind. A `.kb-mcpignore` still sitting in a knowledge base is the one that matters: it is not read, so **whatever it excluded that your current rules do not is in the index**, with nothing anywhere saying so. `doctor` names the file, not what leaked — telling you that would mean reading it and re-running the exclusion decision, and a `doctor` that computes its own answer to a question the indexer already answers is exactly what it must not be. `doctor` reports it along with a stale `.kb-mcp.db`. Those two and no others, because their locations are the ones **nothing can move**: `.grooveignore` is read from a constant name in the knowledge base root, and the index is wherever `resolve_db_path` puts it. `groove.toml` and the old eval filenames are left out for the same reason as each other — their paths are discovered or configurable, so a file at the old name may be the one actually in use, and telling you to rename it would break it.

**When the file that replaced one of these is there too, the finding says something different and advises something different.** `doctor` opens neither file, so with a `.grooveignore` beside the old one all it knows is that the name is taken — not that anything is leaking, and not that the new file is in effect (one that is a directory or over the size cap is refused and applies nothing). What changes is the advice: "rename it to `.grooveignore`" would be telling you to write over whatever is there.
