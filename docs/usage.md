...
It also reports one thing about the knowledge base rather than the index. The ignore file was called `.kb-mcpignore` before [ADR-0007](decisions/0007-rename-the-project-to-grooveseek.md) renamed the project, and an old one left in place is not read — so whatever it used to keep out is indexed again. `doctor` compiles it, asks it about the documents that are actually in the index, and names the ones it matches. It stays quiet when the answer is none, and says so plainly when the check could not be completed — the file is there and cannot be read, or the filesystem will not say whether it is there at all — because that is a different answer from "nothing found". This check is here for the migration and **goes away in 1.1.0**.
