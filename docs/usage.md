...
endpoint to open, validate or serve an index; the reasoning is in
[ADR-0022](decisions/0022-embedding-provider-boundary.md). The one exception
is a forced rebuild: `groove index --force` and the MCP tool `rebuild_index`
...
The `Done in` line counts what the run did: `updated` (files whose content changed and were re-embedded), `renamed`, `deleted`, `skipped` (present on disk but not indexed — unreadable, over the size cap, or empty), and `frontmatter unparsed` (v1.7.0+). That last one counts Markdown files whose YAML frontmatter did not parse this run: each is named in a `warning:` line above, indexed with empty `title` / `date` / `topic`, and tagged `frontmatter:unparsed` so `--tag-any frontmatter:unparsed` lists them. A frontmatter-only stub is skipped for having no content and still counted, since it was still named. A file that has not changed since it was last indexed is not re-parsed and so is not re-counted; `--force` re-parses everything. The exception is the first run of this version over an index an older version wrote: it re-reads the frontmatter of every unchanged Markdown file once, tags and counts what it finds without re-embedding anything, and says so (`Tagged N unchanged Markdown document(s) ...`); later runs take the fast path again. With `[index].fail_on_frontmatter_error = true` in `groove.toml` the run finishes, prints the summary, and then exits 1 when that count is non-zero.
...
Starts the MCP server on stdio transport by default (one client at a time). Pass `--transport http --port <PORT>` (or `--bind <SOCKETADDR>`) to serve multiple clients simultaneously via Streamable HTTP — details in the [HTTP transport](clients.md#http-transport-for-multiple-simultaneous-clients) section. A `--bind` outside loopback additionally requires `--i-know`, because groove ships no authentication. On Linux, where a service manager passes `LISTEN_FDS` — systemd, or anything else speaking the same protocol — `--systemd-socket` takes the listening socket it already bound instead of binding one; it cannot be combined with `--bind` or `--port`, and `serve` refuses to start rather than falling back to TCP when no socket was passed. An abstract socket — a `ListenStream=` whose name starts with `@` — is refused as well, because it has no file for `SocketMode=` to apply to. A build for any other operating system refuses the flag outright. The same applies to `[transport.http].systemd_socket` in a trusted config: if the transport resolves to stdio, which accepts no socket, `serve` refuses to start instead of ignoring the key — unless you asked for stdio yourself with `--transport stdio`, which still runs that config on stdio for the one session.
...
- `jina-v2-ml` — jinaai/jina-reranker-v2-base-multilingual (multilingual, ~1.2 GB). Lighter alternative. **Licensed CC-BY-NC-4.0 by Jina AI: research and evaluation only, no commercial use** — commercial use goes through Jina's API or marketplace offerings. Prefer `bge-v2-m3` (Apache-2.0) for anything commercial.
...
Reports on the existing index, **on stdout**, so `groove status | …` works: document and chunk counts, how many documents had unparseable `tags` frontmatter, the context mode the index was built in (`static` / `off`), and the declared-field set the index recorded (v1.11.0+) — the keys `--field` can filter on, with the number of value rows `document_fields` holds; `pending` means no completed `groove index` run has recorded a set yet, so a search carrying `--field` would be refused right now ([docs/filters.md](filters.md)). A further line reports how many chunks pass the quality filter — only when the effective threshold is above zero, so it is absent under `[quality_filter] enabled = false` or `threshold = 0.0`.
...
It also looks at the declared-field set (v1.11.0+): an index with no recorded set — a run in progress, one that was interrupted, or an index no `groove index` run has completed on since it was created or since it was upgraded from 1.8.0 — is reported as `declared-fields-pending`, because a search carrying `--field` is refused until a run records the set, whether or not a `groove-schema.toml` exists, and even when the index holds no documents yet; and a recorded set that no longer matches what the schema declares is reported as `declared-fields-stale`, because `--field` answers from the recorded set until the next run refreshes the rows. Both are warnings, and `groove index` clears both without re-embedding. A schema file that does not load is treated the way `groove index` and `groove validate` treat it: the command stops (exit `2`) before opening the database.

On Windows it also names documents that an earlier version indexed under a name Windows cannot open as written — a reserved device name such as `CON.md`, a path with a segment ending in a dot or a space, or one holding a character Windows refuses in a name — as `name-not-spellable-on-windows` (a warning). Such names are no longer indexed, so the next `groove index` removes the rows; rename the files to keep them. See [ADR-0023](decisions/0023-index-only-names-the-server-can-open.md).
