...
Starts the MCP server on stdio transport by default (one client at a time). Pass `--transport http --port <PORT>` (or `--bind <SOCKETADDR>`) to serve multiple clients simultaneously via Streamable HTTP — details in the [HTTP transport](clients.md#http-transport-for-multiple-simultaneous-clients) section. A `--bind` outside loopback additionally requires `--i-know`, because groove ships no authentication. Where a service manager passes `LISTEN_FDS` — systemd on Linux, and any other Unix that speaks the same protocol — `--systemd-socket` takes the listening socket it already bound instead of binding one; it cannot be combined with `--bind` or `--port`, and `serve` refuses to start rather than falling back to TCP when no socket was passed. An abstract socket — a `ListenStream=` whose name starts with `@` — is refused as well, because it has no file for `SocketMode=` to apply to. A Windows build refuses the flag.
