...
It also reports one thing about the knowledge base rather than the index. The ignore file was called `.kb-mcpignore` before [ADR-0007](decisions/0007-rename-the-project-to-grooveseek.md) renamed the project, and an old one left in place is not read — so it stops excluding anything at all. What comes back is whatever the two gates that still apply admit: the current exclusion rules, and whether `[parsers].enabled` opens that extension. `doctor` does not reason forward through either of those. It compiles the old file, asks it about the documents **actually in the index**, and names the ones it matches that the current rules do not exclude — so a pattern your `exclude_dirs` or your `.grooveignore` already covers costs you nothing and is never reported. It stays quiet when the answer is none, and says so plainly when the check could not be completed — the file is there and cannot be read, or the filesystem will not say whether it is there at all — because that is a different answer from "nothing found". This check is here for the migration and **goes away in 1.1.0**.

Exit codes: `0` (nothing to report), `1` (findings), `2` (could not run — usually no index). Findings are reported, never repaired: each one names the command that fixes it, which is `groove index` or `groove index --force` for everything structural. The migration check is the one whose remedy starts with a file to move rather than a command to run.
