...
**読まれなくなったファイル**も報告する。本プロジェクトは v0.25.0 まで `kb-mcp` という名前で、改名時に別名も自動移行も**用意しなかった** ([ADR-0007](decisions/0007-rename-the-project-to-grooveseek.ja.md))。`groove` バイナリは `kb-mcp` が残したものを一切開かない。効いてくるのは `.kb-mcpignore` で、**読まれない = そこで除外していたものが全部索引に入っている**のに、それを告げるものがどこにも無い。`doctor` はこれを名指しし、あわせて古い `.kb-mcp.db` と 2 つの旧 eval ファイルも報告する。`groove.toml` は対象外 — 置き場所が固定ではなく**探索で決まる**うえ、設定が読めていなければすぐに分かるため。
