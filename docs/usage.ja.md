...
再ランクは CPU では高い。しかも効いているのはモデルのロードではなく cross-encoder の推論そのものである。v1.0.0 に対して 1 台の Windows マシンで実測 (CPU のみ、埋め込み `bge-m3`、reranker `bge-v2-m3`、141 文書 / 1,855 chunk の KB、`limit = 5` なので候補プールは 50 ペア): 1 クエリが `groove search` で **74〜87 秒**、常駐 daemon の `/mcp` 経由で **74〜79 秒**。同じクエリを再ランク無しで投げると **3.1〜3.6 秒** と **約 0.1 秒**である。**常駐させても救われない** — 起動直後の daemon に投げた最初の再ランク付きクエリ、つまりモデルのロードを payする 1 本が、そのバッチで最速だった。有効にする前に自分のハードウェアで測ること: `groove search "<query>" --reranker bge-v2-m3` と `--reranker none` を同条件で繰り返し、プロセスの外側から時間を計る。`--rerank-by-default <BOOL>` (`--reranker` 指定時は既定 on) はすべての `search` 呼び出しで再ランクするかを制御する。**値を取るフラグ**なので、無効化は `--rerank-by-default=false` と書く。MCP ツール側は `rerank: Option<bool>` で per-query 上書き可能。reranker の切替に**再インデックスは不要** (index 非依存)。
...
索引ではなく**ナレッジベース側**について報告するものが 1 つある。ignore ファイルは [ADR-0007](decisions/0007-rename-the-project-to-grooveseek.ja.md) の改名前は `.kb-mcpignore` という名前で、**残っていても読まれない** — つまり**何も除外しなくなる**。実際に索引へ戻るのは、なお効いている 2 つの gate が通したものだけ: **現在の除外規則**と、**`[parsers].enabled` がその拡張子を開くかどうか**。`doctor` はそのどちらも前向きに辿らない。旧ファイルを compile し、**実際に索引に入っている文書**に当て、マッチしたもののうち**現在の規則が除外しないもの**を名指しする。`exclude_dirs` や `.grooveignore` が既に覆っているパターンは実害が無いので報告されない。該当が無ければ黙り、**検査を完了できなかった場合** — ファイルはあるが読めない、あるいはそもそも在るかどうかをファイルシステムが答えない — は**そう明言する** (「該当なし」とは別の答えなので)。この検査は移行のためのもので、**1.1.0 で無くなる**。

終了コード: `0` (報告なし) / `1` (検出あり) / `2` (実行できない — 大抵は索引が無い)。**報告するだけで修復はしない**。各検出には直し方が併記される (構造的なものはすべて `groove index` か `groove index --force`)。移行の検査だけは、コマンドではなく**ファイルを動かすこと**から始まる直し方になる。
