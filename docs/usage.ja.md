...
**読まれなくなったファイル**も報告する。本プロジェクトは v0.25.0 まで `kb-mcp` という名前で、改名時に別名も自動移行も**用意しなかった** ([ADR-0007](decisions/0007-rename-the-project-to-grooveseek.ja.md))。`groove` バイナリは `kb-mcp` が残したものを一切開かない。効いてくるのは `.kb-mcpignore` で、**読まれない = そこで除外していたもののうち現在の規則が除外しないものが索引に入っている**のに、それを告げるものがどこにも無い。`doctor` が名指しするのは**ファイルであって、漏れた中身ではない** — それを言うにはファイルを読んで除外判定をやり直すことになり、「indexer が既に答えている問いに `doctor` が自前の答えを出す」のは本サブコマンドが最も避けるべき形だから。あわせて古い `.kb-mcp.db` も報告する。**この 2 つだけ**なのは、置き場所を**何者も動かせない**のがこの 2 つだからで、`.grooveignore` は定数名でナレッジベース直下から読まれ、索引は `resolve_db_path` が決める場所にある。`groove.toml` と旧 eval ファイル名が対象外なのは**同じ理由**で、パスが探索または設定で決まる以上、旧名のファイルが**現に使われている**可能性があり、「リネームせよ」がそれを壊しうるため。

**置き換えた側のファイルも並んでいる場合は、言うことも勧めることも変わる。** `doctor` はどちらのファイルも開かないので、`.grooveignore` が隣にある時に分かるのは**その名前が埋まっていること**だけ — 「漏れている」とも「新しい方が効いている」とも言えない (ディレクトリだったりサイズ上限を超えていれば拒否され、1 行も効かない)。変わるのは助言の方で、「`.grooveignore` にリネームせよ」は**そこにあるものを上書きしろ**と言っていることになる。
