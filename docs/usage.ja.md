...
**置き換えた側のファイルも並んでいる場合は、言うことも勧めることも変わる。** `doctor` はどちらの中身も読まないので、現役の `.grooveignore` が隣にあるときに「漏れている」とは断定できない。また「`.grooveignore` にリネームせよ」は、その場合**使用中の設定を上書きしろ**と言っていることになる。
