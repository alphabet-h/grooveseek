...
| `list_topics` | List all indexed topics and categories with document counts. (v1.1.0+) Each entry also carries `children`: the directory tree beneath that category/topic, one node per path segment below the topic, each with `segment`, `file_count` (documents under that prefix, so a parent counts everything beneath it) and its own `children`; root / category-only entries, and topics whose documents sit directly in the topic directory, return `[]`. The tree comes from the stored path, so a document whose frontmatter `topic:` overrides the path-derived topic still contributes the directories after its second path segment. See [MCP surface stability](stability.md#mcp-surface). | (none) |
