...
|---|---|---|
| `search` | Hybrid search (vector + FTS5 full-text) merged via Reciprocal Rank Fusion, optionally followed by cross-encoder reranking, optional MMR diversity re-rank, and optional parent retriever content expansion. Returns a wrapper `{ results, low_confidence, filter_applied }` with chunks ranked by relevance; each result may carry `expanded_from` if parent retriever fired, and — when the chunk came from a source file (v1.2.0+) — `start_line` / `end_line` (1-based, describing the chunk) and `symbol_kind` (the grammar's word for the definition). Those three keys are absent, not `null`, on anything else. See [docs/citations.md](citations.md), [docs/filters.md](filters.md), [docs/retrieval-pipeline.md](retrieval-pipeline.md). | `query` (required; a whitespace-delimited group led by `-`, e.g. `-async`, excludes it from both halves of the search instead of searching for it — v1.1.0+, see [retrieval-pipeline.md](retrieval-pipeline.md) — and the same three-character trigram floor applies, so `-ab` excludes nothing), `limit`, `category`, `topic`, `rerank` (override server default), `min_quality`, `include_low_quality`, `path_globs` (glob list, `!`-prefix excludes), `tags_any` / `tags_all`, `date_from` / `date_to` (`YYYY-MM-DD`), `fields` / `fields_not` (object: key → list of strings, only keys the schema declared, v1.9.0+), `min_confidence_ratio`, `mmr` / `mmr_lambda` / `mmr_same_doc_penalty` (v0.7.0+), `parent_retriever` (v0.7.0+) |
| `list_topics` | List all indexed topics and categories with document counts. (v1.1.0+) Each entry also carries `children`: the directory tree beneath that category/topic, one node per path segment below the topic, each with `segment`, `file_count` (documents under that prefix, so a parent counts everything beneath it) and its own `children`; root / category-only entries, and topics whose documents sit directly in the topic directory, return `[]`. The tree comes from the stored path, so a document whose frontmatter `topic:` overrides the path-derived topic still contributes the directories after its second path segment — and because those segments come from that document's own path, they need not exist as directories under `<category>/<topic>/`, so a client must not build a path prefix out of them. See [MCP surface stability](stability.md#mcp-surface). | (none) |
| `get_document` | Get the full content and metadata of a document by its relative path. The path has to be the document's **one spelling** — relative to the knowledge base, `/`-separated, exactly as `search` returns it. Any other route to the same file (`./a.md`, `a//b.md`, `a/../a.md`, an absolute path, a different case on Windows or macOS, a backslash or an 8.3 short name on Windows, a symlinked directory) is answered "not found", so a gateway that filters on the requested string cannot be walked around. A path that cannot name anything inside the knowledge base — absolute, empty, or holding a `..` segment, and on Windows a drive, a UNC share, a backslash, a colon anywhere (an alternate data stream such as `note.md:secret`) or a reserved device name such as `NUL` or `COM1` — is refused before anything on disk is looked at: the answer is the same whether or not something is there, and on Windows no share is contacted. | `path` (e.g. `"deep-dive/mcp/overview.md"`) |
...
| `rebuild_index` | Rebuild the search index by scanning all source files (Markdown plus any other extensions enabled via `[parsers].enabled`). **One at a time** (v1.0.0+): a call arriving while a rebuild is running is refused with an error naming how long the running one has been going, because a rebuild holds the embedder and the database and search is unavailable until it finishes. The bound is on this tool — `groove index` runs in its own process and is not affected. Returns the same counters as the CLI summary, including `frontmatter_unparsed` (v1.7.0+); under `[index].fail_on_frontmatter_error = true` the reply carries an `error` key beside those counters when it is non-zero. | `force` (optional, default false) |
| `get_connection_graph` | BFS-expand semantically related chunks starting from a document path. Returns a flat list of nodes with `parent_id` / `depth` / `score` / `snippet` so the caller can chain context discovery, plus `truncated` / `truncation[]` when a bound cut the walk short. | `start` (required, the path to walk from — `groove graph --start`), `depth` (default 2, max 3), `fan_out` (default 5, max 20), `min_similarity` (default 0.3), `seed_strategy` (`all_chunks` / `centroid`; `all-chunks` is accepted too), `dedup_by_path`, `category`, `topic`, `exclude_paths`, `max_nodes` (default 100, max 2000), `max_seed_chunks` (default 32, max 1000) |
