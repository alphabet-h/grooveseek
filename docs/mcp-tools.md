...
| `rebuild_index` | Rebuild the search index by scanning all source files (Markdown plus any other extensions enabled via `[parsers].enabled`). **One at a time** (v1.0.0+): a call arriving while a rebuild is running is refused with an error naming how long the running one has been going, because a rebuild holds the embedder and the database and search is unavailable until it finishes. The bound is on this tool — `groove index` runs in its own process and is not affected. | `force` (optional, default false) |
