...
//!   `CREATE_NO_WINDOW`, i.e. exactly the shape a release `kb-mcp-tray.exe` has.
//!
//! `crates/kb-mcp-tray/src/powershell.rs` carries the same three functions. The
//! duplication is deliberate: `kb-mcp` already depends on `kb-mcp-tray`, so the
...
mod tests {
    ...
    /// `crates/kb-mcp-tray/src/powershell.rs` asserts the same text.
    #[test]
