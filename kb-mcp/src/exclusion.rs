...
//!
//! Adding `.kb-mcpignore` on top of two existing layers is exactly how a third
...
//! that must not be readable by kb-mcp belongs outside `kb_path`.
//!
//! That is a deliberate limit rather than an oversight: whoever can write into
//! the knowledge base can also delete `.kb-mcpignore`, so a rule that lives
...
pub const IGNORE_FILE_NAME: &str = ".kb-mcpignore";
...
pub struct ExclusionRules {
    ...
    /// `None` when there is no `.kb-mcpignore`, which is the common case, and
...
impl ExclusionRules {
    /// Read `<kb_path>/.kb-mcpignore`, if it is there, and combine it with
    /// `exclude_dirs`.
    ///
    ...
    /// "excluded" is enough, and `.kb-mcpignore`'s `!` can only undo an earlier
    /// line of `.kb-mcpignore` — never `exclude_dirs`, and never the hardcoded
    ...
    /// Whether a `.kb-mcpignore` is in effect, and with how many patterns.
...
/// Turn the bytes of a `.kb-mcpignore` into a matcher.
///
...
mod tests {
    ...
    fn rules(lines: &str, exclude_dirs: &[&str]) -> ExclusionRules {
        let kb = Path::new("/kb");
        let (ignore, patterns) = compile(kb, Path::new("/kb/.kb-mcpignore"), lines.as_bytes());
        ExclusionRules {
    ...
    fn a_leading_byte_order_mark_is_not_part_of_the_first_pattern() {
        ...
        let (ignore, n) = compile(Path::new("/kb"), Path::new("/kb/.kb-mcpignore"), &bytes);
        assert_eq!(n, 1);
