...
//! is that whoever can write into the KB should not be able to make kb-mcp read
...
//! kb-mcp observes the second name at all. The only check that would close it is
//! ownership — refuse files not owned by the user running kb-mcp — and that
...
mod tests {
    ...
    fn an_ordinary_file_reads_back_byte_for_byte() {
        let tree = TempTree::new("kb-mcp-links-read-ok");
    ...
    fn a_second_name_refuses_the_read_from_either_side() {
        let tree = TempTree::new("kb-mcp-links-read-linked");
    ...
    fn a_hard_link_renamed_over_the_path_after_the_check_is_still_refused() {
        let tree = TempTree::new("kb-mcp-links-toctou");
        let note = tree.join("note.md");
    ...
    fn a_missing_file_is_an_error_and_not_a_refusal() {
        let tree = TempTree::new("kb-mcp-links-read-missing");
    ...
    fn a_directory_is_refused_rather_than_read() {
        let tree = TempTree::new("kb-mcp-links-read-dir");
    ...
    fn the_cap_is_enforced_on_the_handle() {
        let tree = TempTree::new("kb-mcp-links-read-cap");
    ...
    fn a_symlink_swapped_over_the_path_is_refused_rather_than_followed() {
        let tree = TempTree::new("kb-mcp-links-symlink-swap");
        let note = tree.join("note.md");
        std::fs::write(&note, b"ordinary note").unwrap();
    ...
    fn a_fifo_does_not_block_the_open() {
        ...
        let tree = TempTree::new("kb-mcp-links-fifo");
