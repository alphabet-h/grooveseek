...
//! file is *parsed* and *indexed*. Neither answers the question `kb-mcp eval`
...
/// Root holds the KB plus the sibling `.kb-mcp.db` and `kb-mcp.toml`, so the
...
impl TempKb {
    ...
    /// `kb-mcp.toml` next to the KB, enabling every parser under test.
    fn write_config(&self) -> PathBuf {
        let cfg = self.root.join("kb-mcp.toml");
        std::fs::write(
            &cfg,
...
fn eval_retrieves_every_binary_format_from_body_text() {
    let kb = TempKb::new("kb-mcp-eval-binfmt");
    ...
    std::fs::write(kb.kb().join(".kb-mcp-eval.yml"), &golden).unwrap();
