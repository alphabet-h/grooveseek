...
/// The workspace root. `CARGO_MANIFEST_DIR` is `<root>/kb-mcp` since the
/// workspace split (feature-44 PR-1).
fn workspace_root() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .expect("CARGO_MANIFEST_DIR is <workspace root>/kb-mcp, which has a parent")
        .to_path_buf()
}

/// The `name` of every `[[bench]]` entry in kb-mcp's manifest.
...
fn every_bench_target_is_executed_by_the_nightly_workflow() {
    ...
    assert!(
        !declared.is_empty(),
        "no [[bench]] target was found in kb-mcp/Cargo.toml — either they were \
    ...
    assert!(
        missing.is_empty(),
        "these bench targets are declared in kb-mcp/Cargo.toml but never run by \
    ...
    assert!(
        unknown.is_empty(),
        "nightly.yml runs `--bench` on targets that kb-mcp/Cargo.toml does not \
