//! End-to-end coverage for `kb-mcp graph --format`.
//!
...
fn indexed_kb() -> TempKbLayout {
    let kb = TempKbLayout::new("kb-mcp-graph-cli");
    kb.write(
        "rrf.md",
    ...
    let status = Command::new(kb_mcp_bin())
        .arg("index")
        .arg("--kb-path")
        .arg(kb.kb())
        .arg("--model")
...
fn graph(kb: &TempKbLayout, extra: &[&str]) -> std::process::Output {
    let mut cmd = Command::new(kb_mcp_bin());
    cmd.arg("graph")
        .arg("--start")
        .arg("rrf.md")
        .arg("--kb-path")
    ...
    cmd.output().expect("spawn kb-mcp graph")
}
...
fn graph_renders_every_format_to_stdout() {
    ...
    // Default: unchanged by this feature. A caller parsing `kb-mcp graph`
    ...
    assert!(dot.starts_with("digraph kb_mcp_graph {"), "{dot}");
