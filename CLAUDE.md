...
**新規 subprocess test を書く時の注意**: subcommand の出力先を `kb-mcp/src/main.rs` の `Commands::*` block で `println!` (stdout) か `eprintln!` (stderr) かを必ず先に grep 確認すること。**arm が直接書かず helper (`print_search_results` / `print_graph` / `print_validate_report`) に委譲している場合がある**。stdout に書くのは上記 5 subcommand だけで、`index` / `status` / `serve` / `service` は stderr のみ (= F-67 で `kb-mcp status` を stdout から読もうとして fail した過去あり)。
