...
(via: `echo '{"v":"A\rB"}' | jq -r '.v' | od -c`、`... | tr -d '\r' | od -c`、`... | jq -br '.v' | od -c`):
...
> 上の payload の CR は **バックスラッシュ + `r` の 2 文字**で書くこと。JSON は文字列中の生の制御文字を
> 禁じているので、そこに生の CR を置くと `jq` は
> `Invalid string: control characters from U+0000 through U+001F must be escaped` で落ち、
> 表の 3 行はどれも再現しない。**この節自体が一度それを踏んだ** — 編集ツールへ 2 文字のつもりで渡した値が
> 生の CR になり、動かないコマンドを書いた状態で push した (codex round 3 が発見)。
> 罠 11 (エスケープの数え間違いはコンパイルを通ってしまう) と同じ層で、
> **書いたコマンドは file から抜き出して実行するまで信用しない**。
...
## 16. `cargo test` が `LNK1102: メモリ不足です` / `os error 1455` で落ちる (コードは無関係)
...
同族: `cargo test -p grooveseek` の既定並列度で **`os error 1455`** (ページング
ファイルが小さすぎます) が test binary の起動時に出る。link ではなく実行時だが原因は
同じ「同時に走る数」で、`-j 2` で通る (2026-09-09 feature-57 / 58 で実測)。
...
(`-contains` は大文字小文字を区別しないので `B.PDF` も入る)。

**`-Filter` は代わりにならない**: `-LiteralPath` と併用しても効くが、`-Filter '*.pdf'` は
`e.pdfx` / `f.PDFX` にも当たった。Win32 側のワイルドカード照合なので、拡張子の完全一致にならない
...
```
total_files                        = 8
-LiteralPath + -Filter *.pdf       = 4
-LiteralPath + -Include *.pdf      = 8     <- この罠
-LiteralPath + Where .pdf,.txt     = 6
-Path 案件[2026] + -Include *.pdf  = 0     <- [ ] で黙って空
-LiteralPath 案件[2026] + -Filter  = 2

-Filter *.pdf -> a.pdf,B.PDF,e.pdfx,f.PDFX   <- .pdfx にも当たる
Where .pdf    -> a.pdf,B.PDF
```
...
同じ数なら、フィルタが効いていない (対象の拡張子しか置いていない場所では、同じ数になるのが正しい)。
