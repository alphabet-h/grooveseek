...
```bash
S=<scratchpad>
bash .claude/skills/codex-review/scripts/codex_review_round.sh <PR#> 600 > "$S/r1.out" 2> "$S/r1.err"; echo exit=$?
```
...
```bash
bash .claude/skills/codex-review/scripts/codex_review_round.sh <PR#> 600 "$S/r2-body.md" > "$S/r2.out" 2> "$S/r2.err"; echo exit=$?
```

round ごとに別プロセスで、状態は持たない (初回かどうかは `@codex review` の投稿履歴から判定、罠 51)。
...
| `WARN P0/P1 issues present` | blocking な指摘あり | `=== Inline P0/P1 ===` と `=== Top-level summary ===` を読んで fix → push → 次 round。`max_rounds` 到達で user 報告 |
...
| exit 5 | trigger に reaction が 1 つも無い = 届いていない (罠 47、実測 8 回中 2 回) | **round を消費せず**同じ commit に再 trigger (自動 1 回)。2 回目も届かなければ user 報告 |
...
3 round で収束しない = spec / 設計の問題 = user 介入 (軌道修正)。
