...
- **Connection graph**: `get_connection_graph` / `kb-mcp graph` はドキュメント起点でベクトルインデックス上を BFS する。追加インデックスは作らず、**訪問したノードごとに** sqlite-vec KNN を新規発行する。`depth` は 3、`fan_out` は 20 にクランプされるが、**それだけではリクエストのコストを縛れない** — BFS は起点文書の**全チャンク**を種にし、そのチャンク数に上限が無いため。コストは訪問ノード数であり、起点文書のチャンク数と `fan_out^depth` の両方に比例して増える。650 文書の KB で最大の文書 (160 チャンク) に対し release バイナリで実測:
