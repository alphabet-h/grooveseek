...
- **PDF インデックス (v0.10.0+)**: `[parsers].enabled = ["md", "pdf"]` でオプトイン。[oxidize-pdf](https://crates.io/crates/oxidize-pdf) (純 Rust) でページ単位にテキストを抽出し、空でない各ページが見出し `p.N` の 1 チャンクになる。PDF の `Title` / `CreationDate` メタデータがあれば frontmatter に反映、`Title` が無ければファイル名派生タイトルに fallback する。暗号化 PDF は warning 付きで skip (パスワード対応なし)。他のバイナリ形式と同様、`.pdf` にも 50 MiB の生バイト上限が適用され、超過分は実行全体を abort せず warning 付き skip になる。既知の制限:
  - **テキストが薄すぎる PDF は落とす**: 抽出文字数の平均が 50 chars/page 未満の PDF は warning 付きで skip され、一切インデックスされない。スキャン / 画像のみの PDF (**OCR 非対応**) はここに含まれるが、それだけではない — 下記 CJK の制約を参照
  - ⚠️ **日本語をはじめとする CJK の PDF はほぼ機能しない。** oxidize-pdf 4.1.1 に対する 2026-07-28 の実測: CID-keyed な CJK フォントを使う PDF は**文字化けした状態でインデックスされ**て永久にヒットせず、TrueType サブセットを埋め込む PDF (Word / LibreOffice / Google ドキュメントの出力形式) は抽出量が上記閾値を下回って**捨てられる**。`pdfminer.six` は両方とも正しく読めるので、テキスト層は規格通りに存在しており、欠けているのは抽出側である。日本語 PDF は当面使えないものと考えてほしい
