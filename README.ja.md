...
**このテンプレートはコピーしても何も変わらない。** 空ではなく、ファイルの形が見えるように一部のセクション (`[quality_filter]` / `[parsers]` / `[watch]` / `[transport]` / `[transport.http]`) は有効なまま残してあるが、**有効な値はすべて既定値そのもの**なので、コピーは「既定値を明示的に固定する」だけで挙動を変えない。挙動が変わる項目はすべてコメントアウトしてある。一方、下のブロックは各キーが何をするかを示すために**既定値でない値**を入れた形にしてある — **丸ごと貼るためのものではなく、メニューとして**読むこと:
...
# 未知の呼び出し元から kb-mcp の存在を fingerprint されなくなる (v0.7.5+)。
