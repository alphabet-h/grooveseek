...
use groove_tray::powershell::{CREATE_NO_WINDOW, decode_diagnostic, with_utf8_output};
use std::time::Duration;

/// File name of the daemon executable. Every stop is gated on the target
/// process actually being this program.
const DAEMON_IMAGE: &str = "groove.exe";

/// `groove-<service>` is the Task Scheduler task name registered by
/// `groove service install` (= feature-43 `grooveseek/src/service/windows.rs`
...
/// `groove-svc.exe`, which detach-spawns the daemon and exits immediately, so
...
mod tests {
    ...
    #[test]
    fn task_name_uses_grooveseek_prefix() {
        assert_eq!(task_name("groove"), "groove-groove");
        assert_eq!(task_name("work"), "groove-work");
        assert_eq!(task_name("a-b"), "groove-a-b");
    }
    ...
    /// `groove-svc.exe` has already exited by the time the daemon is running,
    /// and terminating it would not stop anything.
    #[test]
    fn daemon_image_is_the_server_not_the_launcher() {
        assert_eq!(DAEMON_IMAGE, "groove.exe");
    }
    ...
    /// exactly that gap. `#[ignore]` because it needs the sibling `groove.exe`
    ...
    /// does run in CI.
    #[tokio::test]
    #[ignore = "spawns a real groove daemon (needs the sibling binary + model cache)"]
    async fn stop_terminates_a_real_daemon_and_confirms_it() {
        let Some(grooveseek) = locate_grooveseek() else {
            panic!("groove.exe not found next to the test binary; run `cargo build` first");
        };
        let dir = scratch_dir("groove-tray-stop-e2e");
        ...
        let mut child = std::process::Command::new(&grooveseek)
            .args(["serve", "--config"])
            .arg(dir.join("groove.toml"))
            .stdin(Stdio::null())
            .stdout(Stdio::null())
            .stderr(Stdio::null())
            .spawn()
        ...
        stop("groove-e2e-no-such-task", &status_url, bind)
            .await
    ...
    /// `groove` lives in a sibling package, so `CARGO_BIN_EXE_` is unavailable.
    /// Walk up from the test binary (`target/<profile>/deps/`) to the profile
    /// directory and look for it there.
    fn locate_grooveseek() -> Option<std::path::PathBuf> {
        let exe = std::env::current_exe().ok()?;
        exe.ancestors().skip(1).take(3).find_map(|dir| {
            let candidate = dir.join("groove.exe");
