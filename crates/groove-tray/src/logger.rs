...
/// `%LOCALAPPDATA%\groove\logs\tray.YYYY-MM-DD`. Override the log level with
/// the `GROOVE_TRAY_LOG` env var (e.g. `GROOVE_TRAY_LOG=debug`).
///
...
pub fn init_file_logger() -> anyhow::Result<()> {
    ...
    let filter =
        EnvFilter::try_from_env("GROOVE_TRAY_LOG").unwrap_or_else(|_| EnvFilter::new("info"));
