...
//! established by feature-43 (grooveseek/src/service/windows.rs), so no new
...
pub fn build_install_script(
    ...
    let lnk_name = ps_quote(&format!("groove-tray-{}.lnk", service_name));
    ...
    let desc = ps_quote(&format!("groove tray monitor for service {}", service_name));
...
pub fn build_duplicate_check_script(service_name: &str) -> String {
    let lnk_name = ps_quote(&format!("groove-tray-{}.lnk", service_name));
    let run_name = ps_quote(&format!("groove-tray-{}", service_name));
    let task_name = ps_quote(&format!(r"\groove-tray-{}", service_name));
    format!(
...
/// set). Used by `groove service install --with-tray` to validate the
...
mod tests {
    ...
    fn install_script_contains_required_lines() {
        let s = build_install_script(
            "groove",
            &PathBuf::from(r"C:\Users\x\.cargo\bin\groove-tray.exe"),
            &PathBuf::from(r"C:\Users\x\AppData\Roaming\groove\groove"),
        );
        ...
        assert!(s.contains("--service-name groove"));
        assert!(s.contains("groove-tray-groove.lnk"));
    ...
    fn install_script_escapes_apostrophe() {
        let s = build_install_script(
            "groove",
            &PathBuf::from(r"C:\Users\O'Brien\bin\groove-tray.exe"),
            &PathBuf::from(r"C:\Users\O'Brien\AppData\Roaming\groove\groove"),
        );
    ...
    fn uninstall_script_is_idempotent() {
        ...
        assert!(s.contains("groove-tray-work.lnk"));
    }
    ...
    fn duplicate_check_emits_three_signals() {
        let s = build_duplicate_check_script("groove");
