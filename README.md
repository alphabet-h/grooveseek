...
> **A language whose grammar is not compiled in needs one more archive.** Rust is built into `groove`; Python arrives as a separate download (v1.3.0+) — `groove-grammar-python-<target>.tar.xz`, or `.zip` on Windows, built for the same four targets as above. Unpack the library into the directory `grammar_dir` names, then add `"py"` to `[parsers].enabled`. Nothing is downloaded automatically, and no library is opened unless that key names the language it belongs to — see [Placing a grammar plugin](docs/clients.md#placing-a-grammar-plugin-v130).
