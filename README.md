...
- **`Origin` validation covers `/mcp`, and `/ui` searches through `/mcp`.** So this list decides whether the built-in page can search: replace the default with only a public origin and `/ui` is still served but every query it makes is refused. `allowed_hosts` does the same one step earlier — Host validation runs first, so the documented LAN recipe (`allowed_hosts = ["kb.example.lan"]`) refuses the `Host: localhost` a locally opened `/ui` sends. Either key **replaces** its default rather than extending it, so keep the loopback entries alongside the public ones if you use `/ui` on the server. The startup log names whichever of the two would break it; the browser only sees a 403 and cannot tell them apart. `/api/admin/status` has no `Origin` check of its own — what restricts it is the requirement that the peer be loopback, which is not configurable.
