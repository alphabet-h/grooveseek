...
- **Connection graph**: `get_connection_graph` / `kb-mcp graph` do BFS over the vector index starting from a document. No extra index is built; **every visited node runs one fresh sqlite-vec KNN**. `depth` is clamped to 3 and `fan_out` to 20, but those do not bound the request on their own, because the walk seeds from **every chunk of the start document** and that count is uncapped. The cost is therefore the number of nodes visited, which scales with the start document's chunk count as well as with `fan_out^depth`. Measured on a 650-document knowledge base, against its largest document (160 chunks), with the release binary:
