...
//! `benches/` can drive internal groove APIs directly. `mmr_perf.rs`
//! does that for `grooveseek::mmr::mmr_select` (Option 1: lib API direct
//! call). `search_latency.rs` still spawns the groove binary as a
...
//!     stdlib or compiler that affect groove's hot path.
//!
