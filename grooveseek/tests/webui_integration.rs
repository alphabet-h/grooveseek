...
mod with_helpers {
    ...
    use grooveseek::server::KbServerShared;
    use grooveseek::transport::http::build_router_for_test;
    ...
    fn build_test_shared(prefix: &str) -> Arc<KbServerShared> {
        ...
        let db_path = tmp.path().join(".groove.db");
        let db = grooveseek::db::Database::open(&db_path.to_string_lossy()).unwrap();
        let embedder = grooveseek::embedder::Embedder::with_model(
            grooveseek::embedder::ModelChoice::default(),
        )
        ...
        // the .groove.db until the KbServerShared is dropped).
        std::mem::forget(tmp);
