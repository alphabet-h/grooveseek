...
//!
//! Spawns `groove index` against a tiny KB containing one h2 + one h3 heading
...
//!
//! `#[ignore]` because `groove index` initialises the embedding model
...
fn index_persists_chunk_level_for_h2_h3() {
    let layout = TempKbLayout::new("groove-parser-level-smoke");
    ...
    let exe = env!("CARGO_BIN_EXE_groove");
    let status = Command::new(exe)
        .args(["index", "--kb-path", &layout.kb().display().to_string()])
        .status()
        .expect("spawn groove index");
    assert!(status.success(), "groove index failed: {status:?}");

    // The DB lands at `kb_path.parent()/.groove.db` (= layout.root()/.groove.db).
