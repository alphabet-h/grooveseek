...
//! `groove serve --transport http` subprocess, mirroring the pattern in
...
#[test]
#[ignore = "spawns groove serve which loads embedding model"]
fn test_parent_expanded_from_set_when_enabled() {
    let layout = TempKbLayout::new("groove-parent-it-expanded");
...
fn test_parent_match_spans_recomputed_on_expanded_content() {
    let layout = TempKbLayout::new("groove-parent-it-spans");
    build_test_kb(&layout);
    build_index(layout.kb());
