...
//! `groove serve --transport http` subprocess, because that is the only
//! call-site where MMR is actually plumbed (CLI `groove search` parses
...
//! - a built `groove` binary (`cargo build` first)
...
//!    `groove.toml` says `[search.mmr] enabled = false`, the request
...
fn test_mmr_off_matches_legacy_search_chunk_id_order() {
    let layout = TempKbLayout::new("groove-mmr-it-off");
...
fn test_mmr_per_call_override_beats_toml() {
    let layout = TempKbLayout::new("groove-mmr-it-override");
    build_test_kb(&layout);
    build_index(layout.kb());
...
fn test_mmr_lambda_warn_when_mmr_off() {
    let layout = TempKbLayout::new("groove-mmr-it-warn");
    build_test_kb(&layout);
    build_index(layout.kb());
