...
//!
//! groove's policy (CLAUDE.local.md, "運用上の気付き") is to **not** depend
...
//! location and does not care about groove's `kb_path.parent()` DB
...
//! `kb_path` you pass to `groove`). groove's runtime places the SQLite
//! file at `kb_path.parent()/.groove.db`, so when the test wants the
...
/// `kb_path` to pass to `groove`.
///
/// Use this when the test wants groove's `.groove.db` (which lands at
...
impl TempKbLayout {
    ...
    /// Returns the path to pass as `--kb-path` to `groove`.
    pub fn kb(&self) -> &Path {
        &self.kb
    }
...
// ---------------------------------------------------------------------------
//
// These run as integration tests of the helpers (not of groove), so we
...
mod tests {
    ...
    fn test_temproot_unique_paths_in_same_thread() {
        let a = TempRoot::new("groove-test-temproot-unique");
        let b = TempRoot::new("groove-test-temproot-unique");
        assert_ne!(a.path(), b.path(), "two TempRoots must not collide");
    }

    #[test]
    fn test_temproot_write_creates_parents() {
        let t = TempRoot::new("groove-test-temproot-nested");
    ...
    fn test_temproot_drop_removes_tree() {
        let captured;
        {
            let t = TempRoot::new("groove-test-temproot-drop");
            t.write("x.md", "y");
    ...
    fn test_tempkblayout_kb_is_under_root() {
        let t = TempKbLayout::new("groove-test-layout");
    ...
    fn test_tempkblayout_drop_removes_root() {
        let captured;
        {
            let t = TempKbLayout::new("groove-test-layout-drop");
