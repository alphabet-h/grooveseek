//! `groove --config <path>` global flag の e2e。tests/validate_cli.rs の TempKb
...
fn grooveseek_bin() -> Option<PathBuf> {
    ...
    // CARGO_BIN_EXE_groove (= cargo が test build 時に absolute path を set
...
impl TempDir {
    fn run_grooveseek(&self, args: &[&str]) -> std::process::Output {
        let bin = grooveseek_bin().expect("groove binary must be built");
        let mut cmd = Command::new(bin);
        cmd.current_dir(&self.path);
        cmd.args(args);
        cmd.output().expect("spawn groove")
    }
}
...
fn test_explicit_config_missing_fails_fast() {
    ...
    let dir = TempDir::new("groove-disc-explicit-miss");
...
fn test_explicit_config_takes_priority_over_cwd() {
    ...
    let dir = TempDir::new("groove-disc-prio");
    dir.write("groove.toml", "kb_path = \"/should-not-win\"\n");
...
fn test_cwd_picked_when_no_explicit() {
    ...
    let dir = TempDir::new("groove-disc-cwd");
    ...
    let out = dir.run_grooveseek(&["status"]);
    let err = stderr_str(&out);
    let plain = strip_ansi(&err);
...
fn test_walks_to_git_root() {
    ...
    let dir = TempDir::new("groove-disc-git");
...
fn test_default_when_no_toml_anywhere() {
    ...
    let dir = TempDir::new("groove-disc-none");
    // CWD / .git / バイナリ隣 (binary はリポジトリ target 配下なので、そちらに groove.toml が
    // 偶然あると AlongsideBinary 経路に入ってしまう)。
    // → リポジトリ target 配下の groove.toml は staging 禁止 (.gitignore 済) なので
