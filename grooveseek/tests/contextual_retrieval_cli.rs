...
//!    reachable via `groove index`, so it piggybacks on scenario 2's
//!    `#[ignore]` test instead of adding a second model-requiring test.
//!
//! All tests that call `groove index` or `groove search` are `#[ignore]`
...
/// Run `groove --config <cfg> index --kb-path <kb> [--force]`, asserting
...
fn test_contextual_on_search_hits_via_context_and_status_shows_static() {
    let layout = TempKbLayout::new("groove-ctx-on");
...
fn test_off_db_config_flip_warns_then_force_migrates_to_static() {
    let layout = TempKbLayout::new("groove-ctx-force");
    layout.write("guide.md", GUIDE_MD);
    let cfg = layout.root().join("groove.toml");
...
fn test_legacy_v0_11_0_db_status_migrates_without_model_download() {
    let layout = TempKbLayout::new("groove-ctx-legacy-status");
...
fn test_legacy_v0_11_0_db_search_works_after_fts_migration() {
    let layout = TempKbLayout::new("groove-ctx-legacy-search");
    let db_path = layout.root().join(".groove.db");
    create_legacy_v0_11_0_db(&db_path);
    let cfg = layout.root().join("groove.toml");
...
fn test_frontmatter_title_change_forces_reembed_but_tags_only_change_skips_it() {
    let layout = TempKbLayout::new("groove-ctx-e8");
...
/// `groove index` and is already asserted by the `#[ignore]`d
...
fn test_status_shows_context_mode_static_without_model_download() {
    let layout = TempKbLayout::new("groove-ctx-status-nomodel");
    let db_path = layout.root().join(".groove.db");
...
fn test_static_mode_rename_no_title_reembeds_and_updates_context() {
    let layout = TempKbLayout::new("groove-ctx-f3-static");
...
fn test_off_mode_rename_no_title_keeps_fast_path() {
    let layout = TempKbLayout::new("groove-ctx-f3-off");
    layout.write("old-widget-doc.md", NO_TITLE_MD);
    let cfg = layout.root().join("groove.toml");
    std::fs::write(&cfg, CONTEXTUAL_OFF).unwrap();
    let db_path = layout.root().join(".groove.db");
...
fn test_static_mode_exclude_heading_wording_change_forces_reembed_and_updates_context() {
    let layout = TempKbLayout::new("groove-ctx-f8-b-static");
...
fn test_off_mode_exclude_heading_wording_change_keeps_fast_path() {
    let layout = TempKbLayout::new("groove-ctx-f8-b-off");
    layout.write("ops-guide.md", EXCLUDE_HEADING_DOC_V1);
    let cfg = layout.root().join("groove.toml");
