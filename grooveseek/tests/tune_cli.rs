//! End-to-end integration test for `groove tune`.
//!
...
fn tune_exits_2_when_no_query_is_fts_effective() {
    ...
    let kb = TempKb::new("groove-tune-it-preflight");
    kb.write(
        "alpha.md",
    ...
    let golden = kb.kb().join(".groove-eval.yml");
...
fn tune_reports_json_for_an_fts_effective_golden() {
    ...
    // grid が回って全セクションが出る。
    let kb = TempKb::new("groove-tune-it-effective");
    for i in 0..4 {
        kb.write(
            &format!("doc_{i}.md"),
            &format!(
