...
//! - **Hand-written, export by export, with no usable table to hand over**, and needing no
//!   grammar crate at all. These are the cases decided before the table is read: a version, a
//!   missing export, a NULL -- the grammar pointer itself included -- a length that does not
//!   describe its pointer, bytes that are not UTF-8. Every one of them is a shape the macro
//!   cannot express, which is why they are written out by hand rather than out of preference.
//! - **Hand-written, and forging a table**, which again needs no grammar crate. Exactly one
//!   check reads the table without building anything from it: whether the Tree-sitter ABI it
//!   declares is one this build speaks. Neither other family can reach it -- the macro only
//!   hands over tables a real grammar crate supplied, and a fixture with no table is refused a
//!   few lines earlier -- so these two hand over a `#[repr(C)]` struct whose first field is the
//!   one Tree-sitter reads. `stale_tree_sitter.rs` carries the argument for why that is sound.
//!
...
// ---------------------------------------------------------------------------
// The grammar's own Tree-sitter ABI has to be one this build speaks
// ---------------------------------------------------------------------------

/// The bounds half of the refusal, built from the constants the loader compares against.
///
/// Derived rather than written out as the numbers they happen to be today: bumping the
/// `tree-sitter` dependency moves both ends, and a literal here would go on pinning a sentence
/// groove has stopped printing. Deriving it is also what lets the tests below claim to check
/// **both** bounds -- a reader who has been refused needs to know whether to go looking for a
/// newer grammar or an older one, and one number cannot tell them.
fn tree_sitter_range() -> String {
    format!(
        "{}..={}",
...
/// A grammar generated by a Tree-sitter older than this build links is refused, not parsed with.
///
/// The check this reaches is the only one that reads the parse table without building anything
/// from it, so it is also the only one no other fixture family can get to: the macro hands over
/// tables real grammar crates supplied, and a fixture with no table is refused a few lines
/// earlier. `stale_tree_sitter.rs` forges one, and carries the argument for why exactly one
/// field of it is ever read.
///
/// The last assertion is the load-bearing one. The fixture declares `rs` while being loaded
/// under `py`, so a build that stopped checking the Tree-sitter version would refuse it a few
/// checks later as a mismatched extension -- a refusal, and a green run, for the wrong reason.
/// Requiring that sentence to be *absent* is what says the version check is what answered.
#[test]
fn a_grammar_from_a_tree_sitter_older_than_this_build_speaks_is_refused_with_both_bounds() {
    let layout = TempKbLayout::new("groove-plugin-stale-ts");
    layout.write("notes.md", SAMPLE_MD);
    let grammars = empty_grammar_dir(&layout);
    place_plugin(&grammars, "groove_grammar_stale_tree_sitter");
    let cfg = write_config(&layout, Some(&grammars));

    let (ok, stderr) = run_index(&cfg, layout.kb());
    assert!(!ok, "an unsupported tree-sitter ABI must fail:\n{stderr}");
    assert!(
        stderr.contains("its tree-sitter ABI version is"),
        "expected the tree-sitter ABI to be named as the reason:\n{stderr}"
    );
    assert!(
        stderr.contains(&tree_sitter_range()),
        "expected both ends of the supported range:\n{stderr}"
    );
    assert!(
        !stderr.contains("but the id it was loaded for stands for"),
        "the run reached the extension check, so the version is not what refused it:\n{stderr}"
    );
    assert!(!db_path(&layout).exists());
}

/// The same refusal from the other side of the range.
///
/// Not a duplicate of the test above: the loader asks `found < min || found > max`, and one
/// fixture leaves whichever half it does not use free to be deleted with every test still
/// green. This is also the side a real user lands on, since a plugin is downloaded on its own
/// and nothing makes the CLI that generated it older than the runtime groove links.
#[test]
fn a_grammar_from_a_tree_sitter_newer_than_this_build_speaks_is_refused_with_both_bounds() {
    let layout = TempKbLayout::new("groove-plugin-future-ts");
    layout.write("notes.md", SAMPLE_MD);
    let grammars = empty_grammar_dir(&layout);
    place_plugin(&grammars, "groove_grammar_future_tree_sitter");
    let cfg = write_config(&layout, Some(&grammars));
