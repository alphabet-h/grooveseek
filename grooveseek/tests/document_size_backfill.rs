...
//! **upgrade**: a knowledge base indexed by an older groove has the column
//! added on open, with every row NULL, and the obvious next step (`groove
...
//!
//! `#[ignore]` because `groove index` loads the BGE-small embedding model
...
fn an_index_run_records_sizes_and_fills_them_in_for_documents_it_skips() {
    let layout = TempKbLayout::new("groove-size-backfill");
...
fn a_file_renamed_in_the_migrating_run_still_gets_its_size_recorded() {
    let layout = TempKbLayout::new("groove-size-rename");
    layout.write("a.md", NOTE_A);
    layout.write("b.md", NOTE_B);
    let bin = grooveseek_bin();
    let db_path = grooveseek::resolve_db_path(layout.kb());
