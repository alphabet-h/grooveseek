//! Subprocess tests for `groove index --quiet` / `--progress` flags.
...
///
/// `TempKbLayout` を使う理由: `resolve_db_path` が `kb_path.parent()/.groove.db`
/// を返すため、`TempRoot` (= 1 階層) だと .groove.db が system temp_dir 直下に
/// 落ちて並列 test で SQLite lock 競合する。`TempKbLayout` は
/// `<temp>/root-unique/kb/` 構造で db が `<temp>/root-unique/.groove.db` に
/// 落ちるため test ごとに分離される。
fn build_small_kb() -> TempKbLayout {
    let kb = TempKbLayout::new("groove-progress-cli");
    for i in 1..=5 {
        kb.write(
            &format!("doc-{i:03}.md"),
...
fn grooveseek_binary() -> PathBuf {
    // Workspace 化 (feature-44 PR-1) 以降、CARGO_MANIFEST_DIR は groove/
    // を指すが、Cargo は binary を workspace root の target/ に置く。
    // CARGO_BIN_EXE_groove は cargo が test build 時に絶対 path を set する
