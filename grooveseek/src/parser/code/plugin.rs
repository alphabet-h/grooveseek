...
pub(crate) fn load(
    ...
    // **The parse table is asked for once and checked before it is used.** Everything below
    // reads through it, starting with `abi_version`, and `ts_language_abi_version` dereferences
    // what it is handed with no check of its own -- so a plugin returning NULL here used to end
    // the process rather than be refused, the one malformed shape this module could not report.
    // Calling the export a second time through `LanguageFn` is free and returns the same
    // pointer: the function the Tree-sitter CLI generates hands back a `static`.
    //
    ...
    // `groove_grammar_language` is contracted to return, and is known non-NULL. A library that
    // lies about the rest is native code the user chose to place, and is trusted to the same
    // degree the binary itself is.
