//! バイナリと同じディレクトリに配置する `groove.toml` の読み込み。
//!
...
/// basename 完全一致 (substring や glob ではない)。`groove.toml` の
...
/// バイナリと同じディレクトリに置く `groove.toml` の表現。
...
pub struct Config {
    ...
    /// `groove eval` (retrieval quality evaluation) の opt-in 設定。
    /// 省略時 (`None`) は全デフォルト値で走る。詳細は `docs/eval.md`。
    pub eval: Option<EvalConfig>,
    /// `groove search` / MCP `search` ツールの設定。
...
/// `groove eval` (retrieval quality evaluation) の設定。省略時は全デフォルトで走る。
...
/// 注入を有効化する。既定 off ―― groove の素の default 構成 (reranker なし) で
...
impl Config {
    ...
    /// なければ CWD → `.git` 祖先 → バイナリ隣の順で `groove.toml` を探し、
    ...
    fn discover_located(
        ...
        // 2. CWD 直下
        let cwd_toml = cwd.join("groove.toml");
        if cwd_toml.exists() {
        ...
        if let Some(root) = find_git_root(cwd) {
            let git_toml = root.join("groove.toml");
            if git_toml.exists() {
    ...
    fn restrict_untrusted(
        ...
        // (`/tmp/groove-fastembed` 等) を発明するのは、まさにここで潰そうと
        ...
        // 優先される (documented) ので、`groove validate --kb-path /safe` が
...
/// `groove.toml` がどのソースから読まれたかを表す。`tracing` ログと
/// テストの assert から参照される。
...
/// 発見した `groove.toml` を**運用者が書いたもの**として扱ってよいか。
///
...
pub enum ConfigTrust {
    ...
    Trusted,
    /// groove が自分で見つけた = **CWD とその `.git` 祖先**。そこに何が置かれて
...
impl TrustRoots {
    /// production の値。
    ///
    /// - `GROOVE_CONFIG_HOME` (service 層が同じ変数で config home を移せる)
    /// - `dirs::config_dir()/groove` — `groove service install` が
    ///   `<config_home>/groove.toml` を書き、各 backend が WorkingDirectory を
    ///   そこに設定して `--config` 無しで起動する。**インストール済み daemon が
    ///   Trusted であり続けるのはこの根による**。service 名まで含めず `groove`
    ///   の階層で見るのは、`<config_dir>/groove/<service>/` 配下すべてを
    ///   1 本で拾うため。
    pub fn from_env() -> Self {
        Self::from_bases(
            env_dir("GROOVE_CONFIG_HOME"),
            dirs::config_dir(),
            dirs::home_dir(),
            dirs::cache_dir(),
    ...
    pub(crate) fn from_bases(
        ...
        // `GROOVE_CONFIG_HOME=$HOME` なら home 配下の全リポジトリが信頼されて
...
mod tests {
    ...
    fn test_load_missing_file_returns_empty() {
        let tmp = std::env::temp_dir().join("groove-nonexistent-config.toml");
        // 念のため存在しないことを確認
    ...
    fn test_parse_full_config() {
        ...
        let mut file = tempfile("groove-config-full");
        writeln!(
            file,
            "kb_path = \"{kb}\"\n\
             model = \"bge-m3\"\n\
    ...
    fn test_best_practice_config_parses_from_toml() {
        let mut file = tempfile("groove-config-bp");
        writeln!(
            file,
            "[best_practice]\n\
    ...
    fn test_best_practice_section_only_yields_empty() {
        ...
        let mut file = tempfile("groove-config-bp2");
    ...
    fn test_best_practice_explicit_empty_accepted() {
        ...
        let mut file = tempfile("groove-config-bp-empty");
        writeln!(
            file,
            "[best_practice]\n\
             path_templates = []\n"
    ...
    fn test_contextual_config_defaults_enabled_false() {
        // judgment gate (eval-baseline-2026-07-20-context.md): groove の素の
    ...
    fn test_contextual_config_parses_from_toml() {
        let mut file = tempfile("groove-config-contextual");
    ...
    fn test_contextual_config_parses_enabled_true_from_toml() {
        let mut file = tempfile("groove-config-contextual-on");
        writeln!(file, "[contextual]\nenabled = true\n").unwrap();
        let cfg = Config::load_from(file.path()).unwrap();
        let c = cfg.contextual.expect("contextual must be Some");
        assert!(c.enabled);
    ...
    fn test_contextual_section_only_defaults_enabled_false() {
        let mut file = tempfile("groove-config-contextual-only");
        writeln!(file, "[contextual]\n").unwrap();
        let cfg = Config::load_from(file.path()).unwrap();
    ...
    fn test_parsers_config_parses_from_toml() {
        let mut file = tempfile("groove-config-parsers");
        writeln!(
            file,
            "[parsers]\n\
             enabled = [\"md\", \"txt\"]\n"
    ...
    fn test_parsers_config_empty_array_is_rejected() {
        ...
        // = ["md"] が適用されて問題ない、という規約。
        let mut file = tempfile("groove-config-parsers-empty");
        writeln!(
            file,
            "[parsers]\n\
             enabled = []\n"
    ...
    fn test_parsers_omitted_uses_md_default() {
        ...
        let mut file = tempfile("groove-config-parsers-omitted");
    ...
    fn test_transport_http_parses() {
        let mut file = tempfile("groove-config-transport-http");
        writeln!(
            file,
            "[transport]\n\
             kind = \"http\"\n\
    ...
    fn test_transport_section_only_http_implies_http_kind() {
        ...
        let mut file = tempfile("groove-config-transport-http-only");
        writeln!(
            file,
            "[transport.http]\n\
             bind = \"127.0.0.1:4567\"\n"
    ...
    fn test_eval_config_parses_from_toml() {
        let mut file = tempfile("groove-config-eval");
        writeln!(
            file,
            "[eval]\n\
             golden = \".groove-eval.yml\"\n\
             history_size = 5\n\
             k_values = [1, 3, 10]\n\
        ...
        assert!(
            golden.ends_with(".groove-eval.yml"),
            "golden should end with .groove-eval.yml, got {golden:?}"
        );
    ...
    fn test_eval_config_omitted_is_none() {
        let mut file = tempfile("groove-config-eval-omit");
        writeln!(file, r#"model = "bge-small-en-v1.5""#).unwrap();
        let cfg = Config::load_from(file.path()).unwrap();
    ...
    fn test_eval_config_rejects_unknown_field() {
        let mut file = tempfile("groove-config-eval-bad");
        writeln!(
            file,
            "[eval]\n\
             bogus = 1\n"
    ...
    fn test_search_config_parses_from_toml() {
        let mut file = tempfile("groove-config-search");
        writeln!(
            file,
            "[search]\n\
    ...
    fn test_search_config_omitted_is_none() {
        let mut file = tempfile("groove-config-search-omit");
        writeln!(file, r#"model = "bge-small-en-v1.5""#).unwrap();
        let cfg = Config::load_from(file.path()).unwrap();
    ...
    fn test_search_config_rejects_unknown_field() {
        let mut file = tempfile("groove-config-search-bad");
        writeln!(
            file,
            "[search]\n\
             bogus = 1\n"
...
"#;
    ...
    fn test_transport_unknown_field_is_rejected() {
        let mut file = tempfile("groove-config-transport-bad");
        writeln!(
            file,
            "[transport]\n\
             bogus = 1\n"
    ...
    fn test_watch_unknown_field_in_config_is_rejected() {
        ...
        let mut file = tempfile("groove-config-watch-bad");
        writeln!(
            file,
            "[watch]\n\
             enabled = true\n\
    ...
    fn test_watch_config_parses_from_toml() {
        let mut file = tempfile("groove-config-watch");
        writeln!(
            file,
            "[watch]\n\
             enabled = false\n\
    ...
    fn test_watch_config_omitted_uses_defaults_via_unwrap_or_default() {
        ...
        let mut file = tempfile("groove-config-watch-omit");
        writeln!(file, r#"model = "bge-small-en-v1.5""#).unwrap();
        let cfg = Config::load_from(file.path()).unwrap();
    ...
    fn test_parsers_unknown_id_is_rejected() {
        let mut file = tempfile("groove-config-parsers-unknown");
        writeln!(
            file,
            "[parsers]\n\
             enabled = [\"md\", \"rst\"]\n"
    ...
    fn test_parse_empty_exclude_headings_overrides_default() {
        ...
        let mut file = tempfile("groove-config-empty-excludes");
    ...
    fn test_parse_partial_config() {
        let mut file = tempfile("groove-config-partial");
        writeln!(file, r#"model = "bge-small-en-v1.5""#).unwrap();
    ...
    fn test_unknown_fields_are_rejected() {
        let mut file = tempfile("groove-config-unknown");
    ...
    fn test_relative_paths_resolve_against_config_dir() {
        ...
        let dir = crate::test_support::unique_temp_path("groove-test-relpath");
        std::fs::create_dir_all(&dir).unwrap();
        let cfg_path = dir.join("groove.toml");
        std::fs::write(
            &cfg_path,
    ...
    fn every_enabled_list_in_the_example_builds_a_registry() {
        let example_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("groove.toml.example");
        ...
        for line in raw.lines() {
            ...
            crate::parser::Registry::from_enabled(&ids).unwrap_or_else(|e| {
                panic!(
                    "groove.toml.example suggests a [parsers].enabled that this build \
                     rejects:\n  {line}\n  {e}"
                )
            });
            checked += 1;
    ...
    /// (BU-13) Copying `groove.toml.example` to `groove.toml` must change
    /// nothing.
    ///
    ...
    fn the_example_as_shipped_changes_no_behaviour() {
        ...
        assert!(
            ...
            "copying groove.toml.example must NOT open a listening socket, but it \
    ...
    fn test_toml_example_parses_with_all_keys_uncommented() {
        ...
        let cfg: Config = toml::from_str(&uncommented).unwrap_or_else(|e| {
            panic!(
                "groove.toml.example failed to parse with all keys enabled: {e}\n\
                 --- generated TOML ---\n{uncommented}\n---"
            )
        });
    ...
    fn test_expand_tilde_with_home() {
        ...
        let expanded = expand_tilde("~/.groove.toml");
        ...
        if let Some(home) = dirs_next_fallback() {
            assert!(
                expanded.starts_with(&home) || expanded == "~/.groove.toml",
    ...
    fn test_find_git_root_returns_dir_when_git_present() {
        let dir = TempDir::new("groove-find-git-root-yes");
    ...
    fn test_find_git_root_handles_git_file_for_worktree() {
        ...
        let dir = TempDir::new("groove-find-git-root-worktree");
    ...
    fn test_find_git_root_returns_none_when_not_in_repo() {
        let dir = TempDir::new("groove-find-git-root-no");
        // `.git` は作らない。
        let found = find_git_root(dir.path());
    ...
    fn test_find_git_root_caps_at_20_levels() {
        ...
        let mut p = std::env::temp_dir().join("groove-cap-test");
        for i in 0..30 {
            p = p.join(format!("d{i}"));
        }
    ...
    fn test_discover_explicit_takes_priority_over_cwd() {
        ...
        let dir = TempDir::new("groove-discover-explicit");
        let cwd_toml = dir.path().join("groove.toml");
    ...
    fn test_discover_explicit_missing_fails_fast() {
        ...
        #[cfg(not(windows))]
        let cwd_kb = "/cwd-kb";
        let dir = TempDir::new("groove-discover-explicit-miss");
        let cwd_toml = dir.path().join("groove.toml");
        std::fs::write(&cwd_toml, format!("kb_path = \"{cwd_kb}\"\n")).unwrap();
    ...
    fn test_discover_cwd_when_no_explicit() {
        ...
        #[cfg(not(windows))]
        let kb = "/cwd-kb";
        let dir = TempDir::new("groove-discover-cwd");
    ...
    fn test_discover_walks_to_git_root() {
        ...
        #[cfg(not(windows))]
        let kb = "/git-kb";
        let dir = TempDir::new("groove-discover-gitroot");
        std::fs::create_dir_all(dir.path().join(".git")).unwrap();
        let toml = dir.path().join("groove.toml");
        std::fs::write(&toml, format!("kb_path = \"{kb}\"\n")).unwrap();
        let nested = dir.path().join("a/b/c");
        std::fs::create_dir_all(&nested).unwrap();
        // CWD = nested (toml 無し)、祖先に .git + groove.toml。
    ...
    fn test_discover_cwd_wins_over_git_root() {
        ...
        let dir = TempDir::new("groove-discover-cwd-vs-git");
        std::fs::create_dir_all(dir.path().join(".git")).unwrap();
        // git root 直下の toml
        std::fs::write(
        ...
        std::fs::write(cwd.join("groove.toml"), format!("kb_path = \"{cwd_kb}\"\n")).unwrap();
    ...
    fn test_discover_returns_default_when_none() {
        let dir = TempDir::new("groove-discover-none");
    ...
    fn test_discover_alongside_binary_when_no_higher_tier() {
        ...
        let cwd_dir = TempDir::new("groove-discover-side-cwd");
        let side_dir = TempDir::new("groove-discover-side-bin");
        let side_toml = side_dir.path().join("groove.toml");
    ...
    fn test_fusion_params_conversion_matches_db_default() {
        ...
        // lib 自身の #[cfg(test)] からは `grooveseek::` は解決できない
        // (lib.rs に `extern crate self as grooveseek` は無い)。`crate::` を使う。
    ...
    // =======================================================================
    // BU-07: a config groove found by itself is not the operator's word
    // =======================================================================
    ...
    fn trust_follows_the_location_and_never_the_contents() {
        let dir = TempDir::new("groove-trust-classify");
        let home = TempDir::new("groove-trust-home");
        let toml = dir.path().join("groove.toml");
        ...
        // What groove found by itself, outside a config home, is not.
        ...
        let daemon_toml = home.path().join("svc").join("groove.toml");
    ...
    fn forbidden_kb_paths_are_the_ones_written_without_knowing_the_username() {
        let repo = TempDir::new("groove-kbpath-repo");
        let home = TempDir::new("groove-kbpath-home");
    ...
    fn a_symlink_kb_path_is_judged_by_its_target() {
        let repo = TempDir::new("groove-kbpath-symlink");
        let home = TempDir::new("groove-kbpath-symlink-home");
        let cfg_dir = repo.path().join("project");
        std::fs::create_dir_all(&cfg_dir).unwrap();
        let roots = roots_for(None, Some(home.path()));
    ...
    fn an_untrusted_config_cannot_choose_which_model_file_is_loaded() {
        let dir = TempDir::new("groove-untrusted-cache");
        std::fs::write(dir.path().join("groove.toml"), planted_toml("kb")).unwrap();
    ...
    fn an_untrusted_config_gets_a_safe_cache_dir_even_when_it_names_none() {
        let dir = TempDir::new("groove-untrusted-no-cache-key");
    ...
    fn the_cache_rule_never_leaves_a_planted_value_in_place() {
        let dir = TempDir::new("groove-untrusted-cache-env");
        std::fs::write(dir.path().join("groove.toml"), planted_toml("kb")).unwrap();
    ...
    /// `service::resolve_config_home` appends `groove/<service>` to whatever
    /// base it resolves — including the `GROOVE_CONFIG_HOME` override. Taking
    /// the override itself as a trust root would make
    /// `GROOVE_CONFIG_HOME=$HOME` trust every repository under the home
    ...
    fn the_trust_root_is_the_grooveseek_subdirectory_of_each_base() {
        ...
        let sibling = base.path().join("some-repo").join("groove.toml");
        assert_eq!(
    ...
    fn an_untrusted_config_cannot_put_the_listener_on_the_network() {
        let dir = TempDir::new("groove-untrusted-bind");
        std::fs::write(dir.path().join("groove.toml"), planted_toml("kb")).unwrap();
        let roots = roots_for(None, None);
    ...
    fn an_untrusted_config_cannot_shrink_the_session_limit() {
        let dir = TempDir::new("groove-untrusted-sessions");
    ...
    fn an_untrusted_config_pointing_at_home_loses_its_kb_path() {
        let dir = TempDir::new("groove-untrusted-kbpath");
        let home = TempDir::new("groove-untrusted-home");
    ...
    fn the_same_file_is_honoured_in_full_once_the_operator_names_it() {
        let dir = TempDir::new("groove-trusted-explicit");
        let home = TempDir::new("groove-trusted-home");
        let toml = dir.path().join("groove.toml");
        let kb = home.path().to_string_lossy().replace('\\', "/");
    ...
    fn a_config_in_the_daemons_config_home_keeps_all_of_its_privileges() {
        let config_home = TempDir::new("groove-daemon-home");
        let svc_dir = config_home.path().join("groove");
        std::fs::create_dir_all(&svc_dir).unwrap();
        let home = TempDir::new("groove-daemon-user-home");
        let kb = home.path().to_string_lossy().replace('\\', "/");
        std::fs::write(svc_dir.join("groove.toml"), planted_toml(&kb)).unwrap();
    ...
    fn the_git_root_tier_is_restricted_too() {
        let root = TempDir::new("groove-untrusted-gitroot");
        std::fs::create_dir_all(root.path().join(".git")).unwrap();
        std::fs::write(root.path().join("groove.toml"), planted_toml("kb")).unwrap();
