...
pub(crate) const LAUNCHD_STDOUT_LOG: &str = "groove.out";
pub(crate) const LAUNCHD_STDERR_LOG: &str = "groove.err";
...
pub(crate) const SERVICE_CONFIG_FILE: &str = "groove.toml";

/// launch line が名指しする config ファイルの絶対パス。
///
/// **なぜ相対パスにしないのか** (BU-07 の残穴): 3 backend とも
/// WorkingDirectory を config home にしているので `--config groove.toml` でも
/// 届くし、quoting の問題も丸ごと消える。それでも絶対パスにするのは、
/// 相対形の失敗モードが**信頼側に倒れる**ため — WorkingDirectory が効かない
/// 環境では、CWD にたまたま在る `groove.toml` を `ConfigSource::Explicit` =
...
/// 設定。空白を含むパスを素で書くと最初の空白で語が切れ、`/home/a b/groove` が
/// コマンド `/home/a` + 引数 `b/groove` になる。
///
...
///   `GROOVE_CONFIG_HOME` に実際に書ける値で、放置すると daemon が
...
pub fn render_unit(ctx: &InstallContext) -> Result<String> {
    ...
    Ok(format!(
        "[Unit]\n\
         Description=groove loopback HTTP MCP server ({name})\n\
...
/// **groove ではなく launchd** で、launchd.plist(5) は `StandardOutPath` /
...
///
/// umask は **job 全体**に効くので、daemon が作る `.groove.db` や WAL も 0600
...
mod tests {
    ...
    fn xml_escaping_leaves_ordinary_paths_byte_for_byte() {
        ...
        for s in [
            "/Users/me/.cargo/bin/groove",
            "/Users/me/Library/Application Support/groove/groove",
            "groove",
            "it's \"fine\" here",
            "日本語/パス",
        ] {
    ...
    fn ordinary_paths_pass_the_unit_value_check() {
        for s in [
            "/home/u/.config/groove/groove",
            "/home/u/.cargo/bin/groove",
            "/home/john doe/kb",
            "/srv/100%/kb",
            "/home/u/it's here",
        ] {
    ...
    fn an_exec_word_without_special_characters_is_left_bare() {
        assert_eq!(
            systemd_exec_word("/home/u/.cargo/bin/groove"),
            "/home/u/.cargo/bin/groove"
        );
    }
    ...
    fn a_space_in_the_binary_path_gets_quoted() {
        assert_eq!(
            systemd_exec_word("/home/john doe/bin/groove"),
            "\"/home/john doe/bin/groove\""
        );
    }
    ...
    /// (systemd.service(5) COMMAND LINES). `GROOVE_CONFIG_HOME='/srv/${TENANT}'`
    ...
    fn a_dollar_sign_is_doubled_so_systemd_does_not_expand_it() {
        assert_eq!(
            systemd_exec_word("/srv/${TENANT}/groove"),
            "/srv/$${TENANT}/groove"
        );
        assert_eq!(systemd_exec_word("/srv/$HOME/groove"), "/srv/$$HOME/groove");
        // Quoting does not stop the expansion, so the doubling has to survive
        // into the quoted form too.
        assert_eq!(
            systemd_exec_word("/srv/${A B}/groove"),
            "\"/srv/$${A B}/groove\""
        );
    ...
    fn the_config_argument_is_dollar_escaped_in_the_unit() {
        let unit = render_unit(&ctx_with("/opt/groove", "/srv/${TENANT}/cfg")).unwrap();
        assert!(
            unit.contains("--config /srv/$${TENANT}/cfg/groove.toml"),
    ...
    fn a_binary_path_with_spaces_is_quoted_but_the_working_directory_is_not() {
        let unit = render_unit(&ctx_with(
            "/home/john doe/bin/groove",
            "/home/john doe/.config/groove/groove",
        ))
        .unwrap();
        assert!(
            unit.contains("ExecStart=\"/home/john doe/bin/groove\" serve"),
            "binary path was not quoted: {unit}"
        );
        // WorkingDirectory で quote が解釈されるかは未確認なので素のまま出す。
        assert!(
            unit.contains("WorkingDirectory=/home/john doe/.config/groove/groove"),
    ...
    fn a_plain_plist_is_rendered_verbatim() {
        ...
        assert!(
            plist.contains("<string>/Users/me/Library/Application Support/groove/groove</string>")
        );
    }

    #[test]
    fn xml_metacharacters_in_a_path_are_escaped_in_the_plist() {
        let plist = render_plist(&ctx_with("/Users/a&b/bin/groove", "/Users/a&b/<cfg>"));
        assert!(
            plist.contains("<string>/Users/a&amp;b/bin/groove</string>"),
    ...
    fn the_plist_makes_launchd_create_the_logs_private() {
        let plist = render_plist(&ctx_with("/Users/me/bin/groove", "/Users/me/cfg"));
        ...
        assert!(
            plist.contains("<string>/Users/me/cfg/groove.out</string>")
                && plist.contains("<string>/Users/me/cfg/groove.err</string>"),
    ...
    /// it *finds* `groove.toml` either way. Naming it is what makes the source
    /// `Explicit` — and therefore Trusted — no matter what `GROOVE_CONFIG_HOME`
    /// was set to at install time and no longer is at run time.
    #[test]
    fn both_unit_and_plist_name_the_config_file_the_installer_wrote() {
        let ctx = ctx_with("/home/u/.cargo/bin/groove", "/home/u/.config/groove/groove");

        let unit = render_unit(&ctx).unwrap();
        assert!(
            unit.contains(
                "ExecStart=/home/u/.cargo/bin/groove serve \
                 --config /home/u/.config/groove/groove/groove.toml"
            ),
        ...
        assert!(
            plist.contains(
                "<string>--config</string>\n        \
                 <string>/home/u/.config/groove/groove/groove.toml</string>"
            ),
    ...
    fn the_config_argument_is_quoted_and_percent_escaped_like_the_binary() {
        ...
        .unwrap();
        assert!(
            unit.contains("--config \"/home/john doe/.config/groove/groove/groove.toml\""),
            "a space in the config path must not split the argument: {unit}"
        );

        let unit = render_unit(&ctx_with("/opt/groove", "/srv/100%/cfg")).unwrap();
        assert!(
            unit.contains("--config /srv/100%%/cfg/groove.toml"),
    ...
    fn xml_metacharacters_in_the_config_path_are_escaped_too() {
        let plist = render_plist(&ctx_with("/Users/me/bin/groove", "/Users/a&b/cfg"));
        assert!(
            plist.contains("<string>/Users/a&amp;b/cfg/groove.toml</string>"),
    ...
    fn no_auto_start_emits_false_for_run_at_load_and_keep_alive() {
        let mut ctx = ctx_with("/Users/me/bin/groove", "/Users/me/cfg");
