//! Linux systemd-user backend for groove service.
//!
...
impl ServiceBackend for SystemdUser {
    fn install(&self, ctx: &InstallContext) -> Result<()> {
        ...
        let name = format!("groove-{}.service", ctx.service_name);
        if ctx.auto_start {
    ...
    fn uninstall(&self, service_name: &str) -> Result<()> {
        let unit_name = format!("groove-{}.service", service_name);
    ...
    fn stop(&self, service_name: &str) -> Result<()> {
        run_systemctl(&["stop", &format!("groove-{}.service", service_name)])
    }
}
