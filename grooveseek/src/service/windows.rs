//! Windows Task Scheduler backend for groove service.
//!
...
///
/// (AU-33) `groove-tray` carries an identical copy in `daemon.rs`, and it
...
///
/// **Invariant, enforced jointly with `crates/groove-svc/src/main.rs`:** exactly
/// one side supplies `serve`. `groove-svc.exe` prepends it unconditionally, so
/// an Action aimed at the svc launcher must leave this empty, while an Action
/// aimed at `groove.exe` must supply it. Breaking either half produces
/// `groove.exe serve serve` (or a bare `groove.exe` with no subcommand) — and
...
///
/// Prefer the windows-subsystem `groove-svc.exe` sibling when present so Task
/// Scheduler spawns a console-less launcher that detach-spawns
/// `groove.exe serve`. Without this the bare `groove.exe` Action surfaces a
...
/// for the parent and the child groove inherits an empty console handle = a true
/// 0-flash launch. When the sibling is absent (e.g. `cargo install --path
/// groove` installs only the main binary) fall back to the legacy
...
pub fn resolve_action_target(binary_path: &Path) -> ActionTarget {
    ...
    match binary_path.parent() {
        Some(dir) => {
            let svc = dir.join("groove-svc.exe");
            if svc.exists() {
                ActionTarget {
                    execute_path: svc,
...
/// with the daemon's stdio nulled by `groove-svc`, which is the least visible
...
///
/// The Action carries `--config <config_home>\groove.toml` (BU-07). Two levels
...
pub fn build_register_script(
    ...
    format!(
        "$ErrorActionPreference='Stop'; \
         ...
         Register-ScheduledTask -TaskName '{name}' -Action $action -Trigger $trigger -Settings $settings -RunLevel Limited -Description 'groove loopback HTTP MCP server ({name})'{force} | Out-Null",
        bin = bin_escaped,
        argument = argument,
        home = home_escaped,
...
/// the `groove-svc.exe` launcher the task detach-spawns the daemon and exits
...
/// error — but it must not stay silent. `groove-svc.exe` was not attached to a
...
fn svc_fallback_warning(binary_path: &Path, auto_start: bool) -> String {
    ...
    format!(
        "warning: groove-svc.exe was not found next to {}.\n         \
         Registered the task to run groove.exe directly, which shows a\n         \
         console window {when}. To avoid it, extract\n         \
         groove-svc-x86_64-pc-windows-msvc.zip from the release next to\n         \
         groove.exe, then re-run the same `groove service install` command\n         \
    ...
    fn list(&self) -> Result<Vec<(String, ServiceState)>> {
        ...
        for line in stdout.lines() {
            if let Some(name_field) = line.split(',').next() {
                ...
                if let Some(rest) = cleaned.strip_prefix("groove-") {
...
mod tests {
    ...
    fn a_service_named_running_is_not_reported_running() {
        ...
        assert!(
            script.contains("-TaskName 'groove-Running').State"),
    ...
    /// the literal. The counterpart is `task_name_uses_grooveseek_prefix` in
    /// `crates/groove-tray/src/daemon.rs`; the two must agree.
    #[test]
    fn task_name_matches_the_tray_copy() {
        assert_eq!(task_name("personal"), "groove-personal");
        assert!(
    ...
    fn svc_fallback_warning_tells_the_user_what_to_do() {
        let msg = svc_fallback_warning(Path::new("C:\\bin\\groove.exe"), true);

        assert!(
            msg.contains("groove-svc-x86_64-pc-windows-msvc.zip"),
        ...
        // A bare `groove service install --force` restores the *default*
        ...
        assert!(
            msg.contains("the same `groove service install` command"),
    ...
    fn svc_fallback_warning_matches_when_the_task_actually_runs() {
        let path = Path::new("C:\\bin\\groove.exe");
        ...
        for msg in [&at_logon, &on_demand] {
            assert!(msg.contains("groove-svc-x86_64-pc-windows-msvc.zip"));
    ...
    fn resolve_action_target_reports_which_branch_it_took() {
        // No `groove-svc.exe` can exist next to a path under a directory that
        // does not exist, so this exercises the fallback without a fixture.
        let missing = Path::new("C:\\groove-au67-no-such-dir\\groove.exe");
    ...
    /// `groove.toml` is `ConfigSource::Explicit` = Trusted regardless of what
    /// `GROOVE_CONFIG_HOME` happened to be set to at install time.
    #[test]
    fn the_action_names_the_config_file_on_both_branches() {
        ...
        let script = build_register_script("groove", &svc, cfg, true, false);
        assert!(
            script.contains("-Argument '--config \"C:\\cfg\\groove.toml\"'"),
        ...
        let script = build_register_script("groove", &console, cfg, true, false);
        assert!(
            script.contains("-Argument 'serve --config \"C:\\cfg\\groove.toml\"'"),
    ...
    fn a_space_in_the_config_home_stays_one_child_argument() {
        ...
        let script = build_register_script(
            "groove",
            &target,
            Path::new("C:\\Users\\John Doe\\AppData\\Roaming\\groove\\groove"),
            true,
            false,
        );
        ...
        assert_eq!(
            ...
            vec![
                "serve".to_string(),
                "--config".to_string(),
                "C:\\Users\\John Doe\\AppData\\Roaming\\groove\\groove\\groove.toml".to_string(),
            ],
    ...
    fn an_apostrophe_in_the_config_home_is_doubled_inside_the_argument_clause() {
        ...
        assert!(
            script.contains("-Argument '--config \"C:\\Users\\O''Brien\\cfg\\groove.toml\"'"),
        ...
        assert_eq!(
            ...
            vec![
                "--config".to_string(),
                "C:\\Users\\O'Brien\\cfg\\groove.toml".to_string(),
            ]
        );
    }
