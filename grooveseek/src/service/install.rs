//! Install orchestration for groove service backends.
use crate::service::{
    InstallContext, ServiceBackend, backend, resolve_config_home, validate_service_name,
};
...
pub struct InstallParams {
    ...
    /// (feature-44 PR-3, Windows-only) Also install the groove-tray.exe
...
///
/// Injecting the backend alone would not be enough: this function creates the
/// config home and writes `groove.toml` into it before the backend is ever
/// consulted, so a test still needs somewhere safe for those to land. The
/// alternative — setting `GROOVE_CONFIG_HOME` — mutates process-wide state, and
/// `cargo test` runs tests on parallel threads of one process (AU-63).
pub(crate) fn run_with_backend(
    ...
    let preflight_tray_exe: Option<PathBuf> = if params.with_tray {
        ...
        #[cfg(target_os = "windows")]
        {
            let bin_dir = binary_path
                .parent()
                .ok_or_else(|| anyhow!("no parent directory for the current groove.exe"))?
                .to_path_buf();
            let tray_exe = bin_dir.join("groove-tray.exe");
            groove_tray::install::preflight_check(&name, &tray_exe, params.force)?;
            Some(tray_exe)
        }
    } else {
        None
    ...
    // CWD before persisting to `groove.toml`. The installed service runs with
    ...
    if let Some(tray_exe) = preflight_tray_exe {
        let lnk = groove_tray::install::install_autostart(&name, &tray_exe, config_home, true)?;
...
pub fn run_tray_install(service_name: &str, force: bool) -> Result<()> {
    ...
    if !tray_exe.exists() {
        return Err(anyhow!(
            "{} not found. Install groove-tray.exe from the groove-tray-x86_64-pc-windows-msvc.zip archive of the matching release, into the same directory as groove.exe.",
            tray_exe.display()
        ));
    }
    let config_home = resolve_config_home(&name)?;
    let lnk = groove_tray::install::install_autostart(&name, &tray_exe, &config_home, force)?;
    eprintln!("Tray autostart shortcut: {}", lnk.display());
    Ok(())
}
...
fn write_toml(path: &std::path::Path, kb_path: &std::path::Path, bind: &str) -> Result<()> {
    // Schema must match `grooveseek::config::Config` (= top-level `kb_path` +
    // `[transport.http]`). `Config` uses `#[serde(deny_unknown_fields)]` so
    // any other section (e.g. `[index]`) would crash `groove serve` at
...
///    (= `grooveseek::config::Config` schema と同じ key、`[index]` は存在しない)
/// 3. 両方 None なら error
    ...
    impl TempDir {
        fn new(tag: &str) -> Self {
            ...
            let dir = std::env::temp_dir()
                .join(format!("groove-write-toml-test-{tag}-{pid}-{nanos}-{seq}"));
...
mod au30_tests {
    ...
    impl TempDir {
        fn new(tag: &str) -> Self {
            ...
            let seq = COUNTER.fetch_add(1, Ordering::Relaxed);
            let dir = std::env::temp_dir()
                .join(format!("groove-{tag}-{}-{nanos}-{seq}", std::process::id()));
            std::fs::create_dir_all(&dir).unwrap();
            Self(dir)
        }
        fn path(&self) -> &Path {
    ...
    fn a_rejected_tray_preflight_leaves_no_toml_behind() {
        ...
        // `target/debug/deps/`, so the `groove-tray.exe` sibling is one
        ...
        assert!(
            msg.contains("groove-tray.exe") || msg.contains("only supported on Windows"),
            "the failure must come from the tray preflight, not elsewhere: {msg}"
        );
        assert!(
            !home.path().join("groove.toml").exists(),
            "a failed preflight must not leave groove.toml behind: {err}"
        );
        assert!(
            !be.0.get(),
    ...
    fn without_tray_the_install_writes_the_toml_and_reaches_the_backend() {
        ...
        assert!(home.path().join("groove.toml").exists());
