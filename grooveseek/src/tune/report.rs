...
/// 採用推奨時に stdout へ出す、そのまま `groove.toml` に貼れるスニペット。
...
pub fn format_text(report: &TuneReport, use_color: bool) -> String {
    ...
    writeln!(s, "{bold}groove tune{reset} — {}", report.kb_path.display()).unwrap();
    writeln!(
        s,
    ...
    match &report.verdict {
        ...
        Verdict::Adopt(c) => {
            ...
            writeln!(
                s,
                "  All adoption conditions passed. Paste this into groove.toml, then re-run\n  \
                 `groove eval` WITH your reranker (and `[contextual]`) enabled to confirm the\n  \
